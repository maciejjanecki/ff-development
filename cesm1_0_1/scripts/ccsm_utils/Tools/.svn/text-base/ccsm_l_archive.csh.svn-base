...
if (-d $DOUT_S_ROOT/${dirl1}/${dirl2} && ${dirl2} != ".") then
  ...
  echo "ccsm_l_archive.csh: now processing files from $DOUT_S_ROOT/${dirl1}/${dirl2}"
  ...
  foreach tarfile ($tarfile_list)
    ...
    if (${status} != 0) then
       echo "ccsm_l_archive.csh: warning, trouble trying to extract existing tar file - continuing"
