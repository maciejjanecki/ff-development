...
#include <mpif.h>
   ...
   type (ESMF_Clock)       :: EClock_d      ! driver clock
   type (ESMF_Clock)       :: EClock_a
   type (ESMF_Clock)       :: EClock_l
   type (ESMF_Clock)       :: EClock_o
   type (ESMF_Clock)       :: EClock_i
   type (ESMF_Clock)       :: EClock_g
...
!-------------------------------------------------------------------------------
   ...
   !-----------------------------------------------------------------------------
   ! Initialize MCT and MPI communicators
   !-----------------------------------------------------------------------------

   call seq_comm_init(NLFileName, atm_petlist=atm_petlist, lnd_petlist=lnd_petlist, &
   ...
   !-----------------------------------------------------------------------------
   ! Initialize ESMF (time manager only)
   !-----------------------------------------------------------------------------
...
#ifdef ESMF_INTERFACE
   call atm_register(atm_petlist)
   call lnd_register(lnd_petlist)
   call ice_register(ice_petlist)
   call ocn_register(ocn_petlist)
   call glc_register(glc_petlist)
#endif
...
!      if (iamroot_CPLID) write(logunit,'(2A)') 'aoflux_grid = ',trim(aoflux_grid)
   elseif (trim(aoflux_grid) == 'atm') then
      if (iamroot_CPLID) write(logunit,'(2A)') 'aoflux_grid = ',trim(aoflux_grid)
      call shr_sys_abort(' aoflux_grid atm not supported yet')
   else
      write(logunit,*) 'ERROR in aoflux_grid = ',trim(aoflux_grid)
      call shr_sys_abort('aoflux_grid error')
   endif
