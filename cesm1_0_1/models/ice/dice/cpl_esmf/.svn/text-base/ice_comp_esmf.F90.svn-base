...
subroutine ice_register_esmf(comp, rc)
   ...
   call ESMF_GridCompSetEntryPoint(comp, ESMF_SETINIT, ice_init_esmf, ESMF_SINGLEPHASE, rc)
   if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
   call ESMF_GridCompSetEntryPoint(comp, ESMF_SETRUN, ice_run_esmf, ESMF_SINGLEPHASE, rc)
   if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
   call ESMF_GridCompSetEntryPoint(comp, ESMF_SETFINAL, ice_final_esmf, ESMF_SINGLEPHASE, rc)
   if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
...
subroutine ice_init_esmf(comp, import_state, export_state, EClock, rc)
   ...
   if (phase == 1) then
      call seq_cdata_init(cdata,ICEID,ggrid,gsmap,infodata,'dice')
   else
