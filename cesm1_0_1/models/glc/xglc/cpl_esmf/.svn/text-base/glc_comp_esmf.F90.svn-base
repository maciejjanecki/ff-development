...
! !USES:
  ...
  use seq_flds_indices , only: nflds_d2x => nflds_g2x, &
                               nflds_x2d => nflds_x2g, &
                               nflds_dom => nflds_dom
...
subroutine glc_register_esmf(comp, rc)
    ...
    call ESMF_GridCompSetEntryPoint(comp, ESMF_SETINIT, &
      glc_init_esmf, ESMF_SINGLEPHASE, rc)
    if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
    call ESMF_GridCompSetEntryPoint(comp, ESMF_SETRUN, &
      glc_run_esmf, ESMF_SINGLEPHASE, rc)
    if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
    call ESMF_GridCompSetEntryPoint(comp, ESMF_SETFINAL, &
      glc_final_esmf, ESMF_SINGLEPHASE, rc)
    if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
...
!EOP
    ...
    if (my_task == master_task) then
       ...
       call shr_file_setIO('glc_modelio.nml',logUnit)
    else
       logUnit = 6
    endif
    ...
    if (my_task == master_task) then
       ...
       open( unitn, file='xglc_in', status='old' )
       read(unitn,*) nxg
       read(unitn,*) nyg
