...
! !USES:
  ...
  use seq_timemgr_mod
  use seq_flds_indices , only: nflds_l2x, &
                               nflds_x2l, & 
                               nflds_r2x, &
                               nflds_x2s, & 
                               nflds_s2x 
...
subroutine dlnd_comp_init( EClock, cdata_l, x2l, l2x, cdata_r, r2x, &
...
!-------------------------------------------------------------------------------
    ...
    if (my_task == master_task) then
       ...
       call shr_file_setIO('lnd_modelio.nml',logUnit)
    else
       logUnit = 6
    endif
   ...
   &                          scmlat=scmlat, scmlon=scmLon)
    ...
    call shr_dmodel_gsmapcreate(gsmap_r,SDROF%nxg*SDROF%nyg,compid,mpicom,decomp)
    ...
    if (rof_present) call shr_dmodel_rearrGGrid(SDROF%grid, dom_r, gsmap_r, rearr_r, mpicom)
...
subroutine dlnd_comp_run( EClock, cdata_l,  x2l, l2x, cdata_r, r2x, cdata_s, x2s, s2x)
...
!   enddo
   ...
   if (trim(rof_mode) /= 'NULL') then
      ...
      call shr_strdata_advance(SDROF,currentYMD,currentTOD,mpicom,'dlnd_r')
      ...
      do n = 1,SDROF%nstreams
         call shr_dmodel_translateAV(SDROF%avs(n),r2x,avifld,avofld,rearr_r)
      enddo
