...
!===============================================================================
  ...
  subroutine lnd_init_mct( EClock, cdata_l, x2l, l2x, cdata_r, r2x, cdata_s, x2s, s2x, NLFilename )
   ...
   if (phase == 1) then
      ...
      call esmf2mct_init(l2xa, LNDID, gsmap_l, mpicom, rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)

      call esmf2mct_init(r2xa, LNDID, gsmap_r, mpicom, rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)

      call esmf2mct_init(s2xa, LNDID, gsmap_s, mpicom, rc=rc)
      if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
