...
subroutine atm_register(atm_petlist)
    ...
    if(rc /= 0) call shr_sys_abort('failed to create atm comp')
    call ESMF_GridCompSetServices(atm_comp, atm_register_esmf, rc=rc)
    if(rc /= 0) call shr_sys_abort('failed to register atm comp')
    import_state = ESMF_StateCreate("atm import", ESMF_STATE_IMPORT, rc=rc)
    if(rc /= 0) call shr_sys_abort('failed to create import atm state')
    export_state = ESMF_StateCreate("atm export", ESMF_STATE_EXPORT, rc=rc)
    if(rc /= 0) call shr_sys_abort('failed to create export atm state')
