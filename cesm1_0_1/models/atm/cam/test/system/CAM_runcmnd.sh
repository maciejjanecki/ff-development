...
#	    cmnd="env OMP_NUM_THREADS=${CAM_THREADS} "
	...
	if grep -ic NOSMP ${CAM_SCRIPTDIR}/config_files/$1 > /dev/null; then
            ##mpi only
	    cmnd="env TARGET_CPU_LIST="-1" LSB_PJL_TASK_GEOMETRY=${geo_string} mpirun.lsf /usr/local/bin/launch "
	else
            ##hybrid
	    cmnd="env TARGET_CPU_LIST="-1" LSB_PJL_TASK_GEOMETRY=${geo_string} OMP_NUM_THREADS=${CAM_THREADS} mpirun.lsf /usr/local/bin/hybrid_launch " 
	fi
    fi ;;
	...
	if grep -ic NOSMP ${CAM_SCRIPTDIR}/config_files/$1 > /dev/null; then
        ...
        if grep -ic NOSMP ${CAM_SCRIPTDIR}/config_files/$1 > /dev/null; then
            ##mpi only
#            cmnd="/usr/local/mpiexec/bin/mpiexec -n ${CAM_TASKS} --comm=mpich-ib "
            cmnd="/usr/local/mpiexec/bin/mpiexec -n ${CAM_TASKS} "
#            cmnd="/usr/local/mvapich-pgi/bin/mpirun -np ${CAM_TASKS} "
        elif grep -ic SMP ${CAM_SCRIPTDIR}/config_files/$1 > /dev/null; then
            ##hybrid
            cmnd="env OMP_NUM_THREADS=${CAM_THREADS} /usr/local/mpiexec/bin/mpiexec -n ${CAM_TASKS} "
        else
            ##mpi only
#            cmnd="/usr/local/mpiexec/bin/mpiexec -n ${CAM_TASKS} --comm=mpich-ib "
            cmnd="/usr/local/mpiexec/bin/mpiexec -n ${CAM_TASKS} "
