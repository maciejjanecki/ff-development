...
if [ -f ${CAM_TESTDIR}/${test_name}/TestStatus ]; then
    if grep -c PASS ${CAM_TESTDIR}/${test_name}/TestStatus > /dev/null; then
        echo "TSM_ccsm.sh: ccsm smoke test has already passed; results are in "
	...
	if [ $JOBID = $prev_jobid ]; then
            echo "TSM_ccsm.sh: ccsm smoke test has already failed for this job - will not reattempt; "
	    echo "        results are in: ${CAM_TESTDIR}/${test_name}" 
	    exit 2
	else
	    echo "TSM_ccsm.sh: this ccsm smoke test failed under job ${prev_jobid} - moving those results to "
...
if [ -f ${CAM_TESTDIR}/case.$1.$2/run/*cam2.h0* ]; then
    rm ${CAM_TESTDIR}/case.$1.$2/run/*cam2.h0*
fi
...
echo "TSM_ccsm.sh: running ccsm; output in ${CAM_TESTDIR}/${test_name}/test.log" 
...
if [ $rc -eq 0 ] && grep -c "SUCCESSFUL TERMINATION" test.log > /dev/null; then
    echo "TSM_ccsm.sh: ccsm smoke test passed" 
    ...
    echo "TSM_ccsm.sh: error running ccsm, error= $rc" 
