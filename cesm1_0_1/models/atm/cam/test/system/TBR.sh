...
master_cam_restart=`ls -1rt ${CAM_TESTDIR}/TSM.$1.$2.${initial_length}${stop_flag}/*.cam2*.r.*[0-9].nc \
    | tail -2 | head -1`
...
echo "        env OMP_NUM_THREADS=${CAM_THREADS} ${cfgdir}/build-namelist -s -runtype branch  \
    ...
    -namelist \"&seq_timemgr_inparm stop_n=${branch_length} stop_option=\'$stop_option\' $decomp_str / \
...
env OMP_NUM_THREADS=${CAM_THREADS} ${cfgdir}/build-namelist -s -runtype branch \
    -config ${CAM_TESTDIR}/TCB.$1/config_cache.xml \
    -config_cice ${CAM_TESTDIR}/TCB.$1/config_cache_cice.xml \
    -ignore_ic_date $use_case_string \
    -infile ${CAM_SCRIPTDIR}/nl_files/$nl_file \
...
for compare_file in ${files_to_compare}; do
	...
	echo "TBR.sh: error from CAM_compare.sh= $rc; see ${rundir}/cprnc.${compare_file} for details" 
