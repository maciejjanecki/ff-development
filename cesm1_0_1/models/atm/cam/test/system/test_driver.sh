...
#
# usage on edinburgh, firefly, bluefire, jaguar:
# ./test_driver.sh
#
# valid arguments: 
# -i    interactive usage
...
##omp threads
export CAM_THREADS=6
export CAM_RESTART_THREADS=12
...
##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ writing to batch script ^^^^^^^^^^^^^^^^^^^
    ...
    if [ -z "$CAM_ACCOUNT" ]; then
	export CAM_ACCOUNT=`showproj -s jaguar | tail -1 `
	if [ -z "${CAM_ACCOUNT}" ]; then
	    echo "ERROR: unable to locate an account number to charge for this job under user: $LOGNAME"
	    exit 2
	fi
...
#PBS -l walltime=11:58:00,size=64
# output file base name
#PBS -N test_dr
# Put standard error and standard out in same file
#PBS -j oe
...
case $arg1 in
    ...
    * )
    echo ""
    echo "**********************"
    echo "usage on edinburgh, firefly, bluefire, jaguar: "
    echo "./test_driver.sh"
    echo ""
