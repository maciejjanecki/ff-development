  ...
  subroutine atm_import_esmf( x2a_a, cam_in ,rc)
    ...
    do c=begchunk,endchunk
       ...
       do i =1,ncols                                                               
          ...
          cam_in(c)%icefrac(i)   =  fptr(index_x2a_Sx_ifrac, ig)  
          cam_in(c)%ocnfrac(i)   =  fptr(index_x2a_Sx_ofrac, ig)
	  cam_in(c)%landfrac(i)  =  fptr(index_x2a_Sx_lfrac, ig)
	    ...
	    cam_in(c)%cflx(i,dust_ndx +3)  = -fptr(index_x2a_Fall_flxdst4, ig)
          ...
          if ( id_isop > 0 .and. index_x2a_Faxx_flxvoc1 > 0 ) then
            cam_in(c)%cflx(i,id_isop)   = -fptr(index_x2a_Faxx_flxvoc1, ig)*isop_factor

          endif
          if ( id_c10h16 > 0 .and. index_x2a_Faxx_flxvoc2 > 0 ) then
            cam_in(c)%cflx(i,id_isop)   = -fptr(index_x2a_Faxx_flxvoc2, ig)*isop_factor
          endif
          if ( index_x2a_Sx_ddvel/=0 .and. n_drydep>0 ) then
             cam_in(c)%depvel(i,:n_drydep) = &
                  fptr(index_x2a_Sx_ddvel:index_x2a_Sx_ddvel+n_drydep-1, ig)
          endif
          !
          ...
          if (index_x2a_Faxx_fco2_lnd /= 0) then
             cam_in(c)%fco2_lnd(i) = -fptr(index_x2a_Faxx_fco2_lnd,ig)
          end if
          if (index_x2a_Faxx_fco2_ocn /= 0) then
             cam_in(c)%fco2_ocn(i) = -fptr(index_x2a_Faxx_fco2_ocn,ig)
          end if
          if (index_x2a_Faxx_fdms /= 0) then
             cam_in(c)%fdms(i)     = -fptr(index_x2a_Faxx_fdms,ig)
          end if
