...
! Public methods
   ...
   public ::&
      ...
      get_calendar,             &! return calendar name
...
! Initialize the clock
   ...
   tm_clock = ESMF_ClockCreate("CAM Time-manager clock", step_size, start_date, &
...
#else
  call ESMF_TimeIntervalSet( diff, d_=time, rc=rc)
#endif
...
! Local variables
   ...
   if ( trim(caltmp) == 'NO_LEAP' .or. trim(caltmp) == 'NOLEAP' ) then
...
! bundy, July 2008
!
   if (( get_curr_calday > 366.0 ) .and. ( get_curr_calday <= 367.0 ) &
        .and. (get_calendar() .eq. 'GREGORIAN')) then
      get_curr_calday = get_curr_calday - 1.0
   endif
...
! bundy, July 2008
!
   if (( get_calday > 366.0 ) .and. ( get_calday <= 367.0 ) &
        .and. (get_calendar() .eq. 'GREGORIAN')) then
...
character(len=SHR_KIND_CS) function get_calendar()

! Return calendar type

! Local variables
   character(len=*), parameter :: sub = 'get_calendar'
!-----------------------------------------------------------------------------------------

   get_calendar = trim(calendar)
...
subroutine timemgr_read_restart(File)
  ...
  do i=1,varcnt
     ...
     if(timevar%name == 'rst_calendar') then
        ...
        if(rst_calendar(1:7).eq.'NO_LEAP') then
           rst_calendar(8:)=' '
        else if(rst_calendar(1:9).eq.'GREGORIAN') then
           rst_calendar(10:)=' '
        end if
     else if(timevar%name == 'rst_nstep') then
