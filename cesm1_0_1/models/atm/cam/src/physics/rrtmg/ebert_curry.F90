...
! 
   integer :: i_dei, i_mu, i_lambda, i_iciwp, i_iclwp, i_cld
...
!    cloud_rad_props_get_lw() is called by radlw() 
   ...
   call ec_ice_get_rad_props_lw(state, pbuf, cld_abs_od, oldicewp=.false.)
...
real(r8) :: wavmax(nswbands) = &  ! Max wavelength (micro-meters) of interval
   ...
   i_rei = pbuf_get_fld_idx('REI')
   icld = pbuf_get_fld_idx('CLD')
   cldn => pbuf(icld)%fld_ptr(1,1:pcols,1:pver,lchnk,itim)
   rei  => pbuf(i_rei)%fld_ptr(1,1:pcols,1:pver,lchnk, 1)
   ...
   if(oldicewp) then
     ...
     cicewp(1:pcols,1:pver) =  1000.0_r8*pbuf(i_iciwp)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
   endif
...
subroutine ec_ice_get_rad_props_lw(state, pbuf, abs_od, oldicewp)
   ...
   do k=1,pver
       do i=1,ncol
          ...
          !   kabsi = 0.005_r8 + 1._r8/(scalefactor*rei(i,k))
          !end if
