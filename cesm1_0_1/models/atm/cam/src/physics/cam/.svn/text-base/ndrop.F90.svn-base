      ...
      real(r8) :: npv(maxd_amode) ! number per volume concentration
      real(r8) :: alogsig(maxd_amode) ! natl log of geometric standard dev of aerosol
      real(r8) :: exp45logsig(maxd_amode)
      real(r8) :: argfactor(maxd_amode)
      real(r8) :: f1(maxd_amode),f2(maxd_amode)  ! abdul-razzak functions of width
...
contains
      ...
      subroutine dropmixnuc(lchnk, ncol, ncldwtr,tendnd, temp,omega,  &
                    ...
                    raer, cflx, raertend, qqcw,  dtmicro)
...
!     output
      ...
      real(r8), intent(inout) :: qqcw(pcols,pver,pcnst) ! cloud-borne aerosol mass, number mixing ratios
...
!      real(r8), parameter :: wmixmin = 1.0 ! minimum turbulence vertical velocity (m/s)
      ...
      integer ntype(maxd_amode)

      real(r8) na(pcols),va(pcols),hy(pcols)
      real(r8) naermod(maxd_amode) ! (/m3)
      real(r8) sigmag(maxd_amode)  ! geometric standard deviation of aerosol size dist
      real(r8) hygro(maxd_amode)  ! hygroscopicity of aerosol mode
      real(r8) vaerosol(maxd_amode) ! interstit+activated aerosol volume conc (cm3/cm3)
      ...
      real(r8) fn(maxd_amode)         ! activation fraction for aerosol number
      real(r8) fm(maxd_amode)         ! activation fraction for aerosol mass

      real(r8) fluxn(maxd_amode)      ! number  activation fraction flux (cm/s)
      real(r8) fluxm(maxd_amode)      ! mass    activation fraction flux (cm/s)
...
!           / [aero. number conc. in updraft, just below cloudbase (#/cm3)]
      ...
      real(r8) nact(pver,maxd_amode)  ! fractional aero. number  activation rate (/s)
      real(r8) mact(pver,maxd_amode)  ! fractional aero. mass    activation rate (/s)
      real(r8) sigmag_amode_cur(maxd_amode) ! sigmag for current grid cell
...
!                 raercol(:,lnumcw,nsav)=raer(i,:,lnumcw)
                  raercol_cw(:,lnumcw,nsav)=qqcw(i,:,lnumcw)   ! use cloud-borne aerosol array
...
!                 raercol(:,lmasscw,nsav)=raer(i,:,lmasscw)
                  raercol_cw(:,lmasscw,nsav)=qqcw(i,:,lmasscw)  ! use cloud-borne aerosol array
...
!    artificial source.
	 ...
	 do k=1,pver
	    ...
	    ! maximum overlap assumption
            if(cldn(i,kp1).gt.0)then
               overlapp(k)=min(cldn(i,k)/cldn(i,kp1),1._r8)
            else
               overlapp(k)=1.
            endif
            if(cldn(i,km1).gt.0)then
...
!        aerosol tendency
         do m=1,ntot_amode
            ...
            lnumcw=numptrcw_amode(m)
	    if(lnum.gt.0)then
               qqcwtend(i,:,lnumcw)=(raercol_cw(:,lnumcw,nnew)-qqcw(i,:,lnumcw))*dtinv
               qqcw(i,:,lnumcw)=raercol_cw(:,lnumcw,nnew)    ! update cloud-borne aerosol
	       raertend(i,:,lnum)= (raercol(:,lnum,nnew)-raer(i,:,lnum))*dtinv
	    endif
	    do l=1,nspec_amode(m)
	       lmass=lmassptr_amode(l,m)
	       lmasscw=lmassptrcw_amode(l,m)
               qqcwtend(i,:,lmasscw)=(raercol_cw(:,lmasscw,nnew)-qqcw(i,:,lmasscw))*dtinv
               qqcw(i,:,lmasscw)=raercol_cw(:,lmasscw,nnew)   ! update cloud-borne aerosol
...
!      local
      ...
      real(r8) xmin(maxd_amode),xmax(maxd_amode) ! ln(r) at section interfaces
      real(r8) volmin(maxd_amode),volmax(maxd_amode) ! volume at interfaces
      real(r8) tmass ! total aerosol mass concentration (g/cm3)
      real(r8) sign(maxd_amode)    ! geometric standard deviation of size distribution
      ...
      real(r8) zeta(maxd_amode), eta(maxd_amode)
      real(r8) lnsmax ! ln(smax)
      real(r8) alpha
      real(r8) gamma
      real(r8) beta
      real(r8) sqrtg(maxd_amode)
      real(r8) :: amcube(maxd_amode) ! cube of dry mode radius (m)
      real(r8) :: smcrit(maxd_amode) ! critical supersatuation for activation
      real(r8) :: lnsm(maxd_amode) ! ln(smcrit)
      real(r8) smc(maxd_amode) ! critical supersaturation for number mode radius
      real(r8) sumflx_fullact
      real(r8) sumflxn(maxd_amode)
      real(r8) sumflxm(maxd_amode)
      real(r8) sumfn(maxd_amode)
      real(r8) sumfm(maxd_amode)
      real(r8) fnold(maxd_amode)   ! number fraction activated
      real(r8) fmold(maxd_amode)   ! mass fraction activated
      real(r8) wold,gold
      real(r8) alogam
      ...
      real(r8) etafactor1,etafactor2(maxd_amode),etafactor2max
      integer m,n
!      numerical integration parameters
      ...
      if(maxd_amode<pmode)then
         write(iulog,*)'maxd_amode,pmode in activate =',maxd_amode,pmode
...
!     input
      ...
      real(r8), intent(in) :: raer(pcols,pver,pcnst) ! aerosol mass, number mixing ratios
      real(r8), intent(in) :: qqcw(pcols,pver,pcnst) ! cloud-borne aerosol mass, number mixing ratios
      ...
      real(r8), intent(in) :: npv(maxd_amode) ! number per volume concentration
      real(r8), intent(in) :: alogsig(maxd_amode) ! natl log of geometric standard dev of aerosol
...
!     local
      ...
      real(r8) exp45logsig(maxd_amode)     ! exp(4.5*alogsig**2)
      real(r8) amcube(pcols)
      real(r8) super(psat) ! supersaturation
      real(r8) amcubecoef(maxd_amode)
      real(r8) :: surften       ! surface tension of water w/respect to air (N/m)
      ...
      real(r8) argfactor(maxd_amode),arg(pcols)
!     mathematical constants
      real(r8) pi
...
!                  vol(i)=max(raer(i,k,lmass)+raer(i,k,lmasscw),0._r8)/specdens_amode(ltype)
                   vol(i)=max(raer(i,k,lmass)+qqcw(i,k,lmasscw),0._r8)/specdens_amode(ltype)
		end do
             elseif(phase.eq.2)then
                do i=istart,istop
                   vol(i)=max(qqcw(i,k,lmasscw),0._r8)/specdens_amode(ltype)
                end do
...
!                  naerosol(i)=(raer(i,k,lnum)+raer(i,k,lnumcw))*cs(i,k)
                   naerosol(i)=(raer(i,k,lnum)+qqcw(i,k,lnumcw))*cs(i,k)
		end do
             elseif(phase.eq.2)then
                do i=istart,istop
                   naerosol(i)=qqcw(i,k,lnumcw)*cs(i,k)
                end do
	     else
	        do i=istart,istop
