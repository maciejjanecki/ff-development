...
!  integer, intent(in)  :: nsrflx               ! last dimension of qsrflx
   ...
   real(r8), intent(inout) :: qqcw(pcols,pver,pcnst) ! cloudborne tracer MR array 
!  real(r8), intent(out)   :: qsrflx(pcols,pcnst,nsrflx)
   ...
   real(r8), intent(inout) :: dgncur_a(pcols,pver,maxd_amode)
...
! local
   ...
   logical  :: noxf_acc2ait(maxd_aspectype)
   ...
   real(r8) :: dgncur_c(pcols,pver,maxd_amode)
   ...
   real(r8) :: drv_a_sv(pcols,pver,maxd_amode), drv_c_sv(pcols,pver,maxd_amode)
   ...
   real(r8) :: num_a_sv(pcols,pver,maxd_amode), num_c_sv(pcols,pver,maxd_amode)
   real(r8) :: pdel_fac                      ! 
   real(r8) :: tadj                          ! adjustment time scale
   real(r8) :: tadjinv                       ! 1/tadj
   real(r8) :: v2ncur_a(pcols,pver,maxd_amode)
   real(r8) :: v2ncur_c(pcols,pver,maxd_amode)
...
! need qmass*dummwdens = (kmol/kmol-air) * [(kg/kmol)/(kg/m3)] = m3/mol-air
      ...
      do i=1,ncol
         dryvol_c(i,k) = dryvol_c(i,k)    &
                         + max(0.0_r8,qqcw(i,k,lc))*dummwdens
      end do
      end do
    end do
...
40000 continue   ! "do n = 1, ntot_amode"
...
!
      ixfer_ait2acc_sv(:,:) = 0
      ixfer_acc2ait_sv(:,:) = 0
      if ( .not. do_aitacc_transfer ) goto 49000
...
! jump to end-of-loop if no transfer is needed at current i,k
      if (ixfer_ait2acc+ixfer_acc2ait .le. 0) goto 48010
...
! j=1 does aitken-->accum; j=2 does accum-->aitken 
      ...
      if (j .eq. 1) then
          if (ixfer_ait2acc <= 0) goto 46900
      else
          if (ixfer_acc2ait <= 0) goto 46900
      end if
      ...
      do 46700 iq = 1, nspecfrm_renamexf(ipair)
            ...
            if ((lsfrm > 0) .and. (lstoo > 0)) then
               if (jac .eq. 1) then
                  ...
                  if (iq .eq. 1) then
                     ...
                     xfertend = max(0.0_r8,qqcw(i,k,lsfrm))*xfercoef
                  end if
...
!+++ start of diagnostics for testing ++++++++++++++++++++++++++++++++++
      if (pver > 0) goto 59000   ! activate this to bypass diagnostics
...
!        if ((lat(i) == plat/2) .and. (lon(i) == plon/2)) icol_diag = i
      ...
      do n = 1, ntot_amode
          ...
          write( *, '(/a,1p,5e11.3)' ) 'num_c_sv, n, dndt, n+del     ',    &
             num_c_sv(i,k,n), qqcw(i,k,lnc), dqqcwdt(i,k,lnc),   &
             (qqcw(i,k,lnc) + deltat*dqqcwdt(i,k,lnc))
          duma = max( drv_c_sv(i,k,n), 1.0d-35 )
          dumb = max( qqcw(i,k,lnc), 1.0d-35 )
          duma = duma**third
          dumb = dumb**third
          if (duma < dumb*1.0e20) then
             dumc = ((6.0/pi)**third)*duma/dumb
...
!
      do l = 1, pcnst
         lc = l - loffset
         if (lc <= 0) cycle
         if ( dotendqqcw(lc) ) then
            do k = 1, pver
            do i = 1, ncol
               qqcw(i,k,lc) = max( 0.0_r8,   &
                            (qqcw(i,k,lc) + dqqcwdt(i,k,lc)*deltat) )
            end do
            end do
         end if
      end do
