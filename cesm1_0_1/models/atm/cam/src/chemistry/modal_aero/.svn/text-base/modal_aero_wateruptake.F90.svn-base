...
!-----------------------------------------------------------------------
      ...
      real(r8), intent(out)   :: qaerwat(pcols,pver,maxd_amode)
      real(r8), intent(in)    :: dgncur_a(pcols,pver,maxd_amode)
      real(r8), intent(out)   :: dgncur_awet(pcols,pver,maxd_amode)
      real(r8), intent(out)   :: wetdens(pcols,pver,maxd_amode)

! following are local for now -- wait and see
!     real(r8), intent(out) :: maer(pcols,pver,maxd_amode)
!                              ! aerosol wet mass MR (including water) (kg/kg-air)
!     real(r8), intent(out) :: naer(pcols,pver,maxd_amode)
!                              ! aerosol number MR (bounded!) (#/kg-air)
!     real(r8), intent(out) :: wetrad(pcols,pver,maxd_amode)  
!                              ! wet radius of aerosol (m)
!     real(r8), intent(out) :: density(pcols,pver,maxd_amode)
...
!     integer lwater ! pointer for water on aerosol
      ...
      real(r8) drydens(maxd_amode)   ! dry particle density  (kg/m^3)
      real(r8) drymass(maxd_amode)   ! single-particle-mean dry mass  (kg)
      real(r8) dryrad(pcols,pver,maxd_amode) ! dry volume mean radius of aerosol (m)
      real(r8) dryvol(maxd_amode)    ! single-particle-mean dry volume (m3)
      real(r8) dryvolmr(maxd_amode)  ! volume MR for aerosol mode (m3/kg)
      real(r8) duma, dumb
      real(r8) es(pcols,pver)        ! saturation vapor pressure (Pa)
      real(r8) hygro(maxd_amode)     ! volume-weighted mean hygroscopicity (--)
      real(r8) hystfac(maxd_amode)   ! working variable for hysteresis
      real(r8) pi43
      real(r8) qs(pcols,pver)        ! saturation specific humidity
      real(r8) qwater                ! aerosol water MR
      real(r8) rh(pcols,pver)        ! relative humidity (0-1)
      real(r8) third
      real(r8) v2ncur_a(pcols,pver,maxd_amode)
      real(r8) wtrvol(maxd_amode)    ! single-particle-mean water volume in wet aerosol (m3)
      real(r8) wetvol(maxd_amode)    ! single-particle-mean wet volume (m3)

      real(r8) :: maer(pcols,pver,maxd_amode)
                              ! aerosol wet mass MR (including water) (kg/kg-air)
      real(r8) :: naer(pcols,pver,maxd_amode)
                              ! aerosol number MR (bounded!) (#/kg-air)
      real(r8) :: wetrad(pcols,pver,maxd_amode)  
                              ! wet radius of aerosol (m)
