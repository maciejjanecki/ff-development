...
subroutine prescribed_ghg_readnl(nlfile)
   ...
   integer            :: prescribed_ghg_ymd
   integer            :: prescribed_ghg_tod
   ...
   prescribed_ghg_ymd      = start_ymd
   prescribed_ghg_tod      = start_tod
   ...
   call mpibcast(prescribed_ghg_ymd,      1, mpiint,  0, mpicom)
   call mpibcast(prescribed_ghg_tod,      1, mpiint,  0, mpicom)
#endif
   ...
   start_ymd  = prescribed_ghg_ymd
   start_tod  = prescribed_ghg_tod
...
!-------------------------------------------------------------------
  subroutine prescribed_ghg_register()
    ...
    if (has_prescribed_ghg) then
       do i = 1,N_GHG
          call pbuf_add(ghg_names(i),'physpkg',1,pver,1,idx)
       enddo
    endif
...
!-------------------------------------------------------------------
  subroutine prescribed_ghg_init()
    ...
    do i = 1,number_flds
       ...
       if (ndx < 1) then
          call endrun('prescribed_ghg_init: '//trim(fields(i)%fldnam)//' is not one of the named ghg fields in pbuf')
       endif
...
!-------------------------------------------------------------------
!-------------------------------------------------------------------
  subroutine prescribed_ghg_adv( state )
    ...
    do i = 1,number_flds
       ...
       do c = begchunk,endchunk
          ...
          fields(i)%data(:ncol,:,c+fields(i)%chnk_offset) = to_mmr(:ncol,:) * fields(i)%data(:ncol,:,c+fields(i)%chnk_offset) 
          call outfld( fields(i)%fldnam, fields(i)%data(:ncol,:,c+fields(i)%chnk_offset), ncol, state(c)%lchnk )
       enddo
    enddo
