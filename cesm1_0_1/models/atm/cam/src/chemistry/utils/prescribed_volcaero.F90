...
subroutine prescribed_volcaero_readnl(nlfile)
   ...
   call mpibcast(prescribed_volcaero_ymd,      1, mpiint,  0, mpicom)
   call mpibcast(prescribed_volcaero_tod,      1, mpiint,  0, mpicom)
#endif
...
!-------------------------------------------------------------------
  subroutine prescribed_volcaero_init()
    ...
    ndx = pbuf_get_fld_idx(volcrad_name, failcode=-1 )
...
!-------------------------------------------------------------------
!-------------------------------------------------------------------
  subroutine prescribed_volcaero_adv( state )
    ...
    do c = begchunk,endchunk
       ...
       call outfld( volcaero_name, fields(1)%data(:,:,c+fields(1)%chnk_offset), pcols, state(c)%lchnk)
       call outfld( volcrad_name, radius(:,:,c+rad_chnk_offset), pcols, state(c)%lchnk)
