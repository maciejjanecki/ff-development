...
!----------------------------------------------------------------------- 
  ...
  use time_manager, only : get_curr_date, get_step_size, get_calendar, get_curr_calday
  ...
  type trfile
     ...
     real(r8), pointer, dimension(:) :: hyai 
     real(r8), pointer, dimension(:) :: hybi
     real(r8), pointer, dimension(:,:) :: weight_x, weight_y
     integer, pointer, dimension(:) :: count_x, count_y
     integer, pointer, dimension(:,:) :: index_x, index_y
     real(r8)                        :: p0
...
!    select case ( to_upper(data_type) )
    ...
    if ( len_trim(file%curr_filename)<1 .or. file%fixed ) then ! initial run
       ...
       if ( data_ymd > 0 .and. .not. (file%cyclical .or. file%cyclical_list)) then
          str_yr = data_ymd/10000
          str_mon = (data_ymd - str_yr*10000)/100
          str_day = data_ymd - str_yr*10000 - str_mon*100
          call set_time_float_from_date( start_time, str_yr, str_mon, str_day, data_tod )
    ...
    flds_loop: do f = 1,mxnflds
       ...
       if ( .not. file%in_pbuf ) then
          ...
          idx = pbuf_get_fld_idx(flds(f)%fldnam, failcode=-1 )
...
! if weighting by laitutde, compute weighting for horizontal interpolation
...
!-----------------------------------------------------------------------
  subroutine advance_trcdata( flds, file, state )
    ...
    call interpolate_trcdata( flds, file, state )
...
!------------------------------------------------------------------------------
  subroutine check_files( file, fids, itms, times_found)
    ...
    integer, intent(out),optional   :: itms(2)
...
! If weighting by latitude, then perform horizontal interpolation by using weight_x, weight_y
   ...
   if(file%weight_by_lat) then
     ...
     do c = begchunk,endchunk
        ...
        call xy_interp(file%nlon,file%nlat,file%nlev,plon,plat,pcols,ncols,file%weight_x,file%weight_y,wrk3d_in,loc_arr(:,:,c-begchunk+1),  &
                            lons,lats,file%count_x,file%count_y,file%index_x,file%index_y) 
     enddo
...
!------------------------------------------------------------------------------
  ...
  subroutine interpolate_trcdata( flds, file, state )
       ...
       elseif (file%alt_data) then
          do f = 1,nflds
             ...
             do i = 1,ncol
                ...
                call rebin( file%nlev, pver, file%ilevs, model_z, datain(i,:), flds(f)%data(i,:,c+flds(f)%chnk_offset) )
             enddo
          enddo
       else
          if ( file%has_ps ) then
          ...
          do f = 1,nflds
             ...
             if ( file%top_bndry ) then
                call vert_interp_ub(ncol, file%nlev, file%levs,  datain(:ncol,:), flds(f)%data(:ncol,:,c+flds(f)%chnk_offset) )
             else if(file%conserve_column) then
               call vert_interp_mixrat(ncol,file%nlev,pver,state(c)%pint, &
                                       datain,flds(f)%data(:,:,c+flds(f)%chnk_offset), &
                                       file%p0,ps,file%hyai,file%hybi)
             else
                call vert_interp(ncol, file%nlev, pin, state(c)%pmid, datain, flds(f)%data(:,:,c+flds(f)%chnk_offset) )
             endif
          enddo
       endif
