...
!-----------------------------------------------------------------------
  ...
  real(r8), parameter :: molmass(N_AERO) = 1.
...
! count aircraft emission species used in the simulation
	...
	if( len_trim(air_specifier(n) ) == 0 ) then
	 ...
	 call endrun('aircraft_emit_register: '//trim(spc_name)//' is not in the aircraft emission dataset')
        ...
        call pbuf_add(aero_names(mm),'physpkg',1,pver,1,idx)
...
!         forcings_air(m)%file%curr_filename    = spc_fname(m)
    ...
    do m=1,aircraft_cnt
       ...
       call trcdata_init( forcings_air(m)%sectors, forcings_air(m)%filename, forcings_air(m)%filelist, datapath, &
                          forcings_air(m)%fields, forcings_air(m)%file, rmv_file, 0, 0, air_type )
  ...
  subroutine aircraft_emit_adv( state)
!-------------------------------------------------------------------
...
! C.-C. Chen
    ...
    integer :: ind,c,ncol,i,caseid,chnk_offset,m,n
...
! C.-C. Chen
   ...
   !------------------------------------------------------------------
    if (aircraft_cnt == 0 ) return
    call t_startf('All aircraft_emit_adv')

   !-------------------------------------------------------------------
   !    For each field, read more data if needed and interpolate it to the current model time
   !-------------------------------------------------------------------
    do m = 1, aircraft_cnt
       call advance_trcdata( forcings_air(m)%fields, forcings_air(m)%file, state)
...
! C.-C. Chen, adding case 4  for kg/sec
          ...
          ind = index_map(i)
          chnk_offset = forcings_air(m)%fields(i)%chnk_offset
...
!                end do
             ...
             forcings_air(m)%fields(i)%data(:ncol,:,c+chnk_offset) = to_mmr(:ncol,:) * &
                                                                  forcings_air(m)%fields(i)%data(:ncol,:,c+chnk_offset)

               
             call outfld( forcings_air(m)%fields(i)%fldnam, forcings_air(m)%fields(i)%data(:ncol,:,c+chnk_offset), ncol, state(c)%lchnk )
          enddo
       enddo
    enddo
