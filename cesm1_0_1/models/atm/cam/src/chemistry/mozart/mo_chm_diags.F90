...
#if (defined MODAL_AERO)
  subroutine chm_diags( lchnk, ncol, vmr, mmr, mmrcw, rxt_rates, invariants, depvel, depflx, mmr_tend, pdel, imozart )
#else
...
#if (defined MODAL_AERO)
    real(r8), intent(in)  :: mmrcw(ncol,pver,gas_pcnst)
...
! output cloud borne aerosol
          n = m + imozart - 1
          if( .not. (cnst_name_cw(n) == ' ') ) then
             call outfld( cnst_name_cw(n), mmrcw(:ncol,:,m), ncol ,lchnk )   ! output as mmr (kg/kg)
          end if
#else
  ...
  subroutine het_diags( het_rates, mmr, pdel, lchnk, ncol )
    ...
    do i = 1,hetcnt
       ...
       wrk_wd(:ncol) = 0._r8
       do k = 1,pver
          wrk_wd(:ncol) = wrk_wd(:ncol) + het_rates(:ncol,k,wd_index) * mmr(:ncol,k,m) * pdel(:ncol,k) * rgrav * wght(:ncol)
       end do
       !
       wrk_wd(:ncol) = wrk_wd(:ncol) * four_pi * rearth**2
       !
