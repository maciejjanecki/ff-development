...
contains
  ...
  subroutine setext( lonndx, latndx, extfrc, zint_abs, zint_rel, cldtop, &
    ...
    use mo_airplane,  only : pno, pco, air_altitude, has_airpl_src
    ...
    integer,  intent(in)  ::   lonndx(pcols)               ! long indicies in chunk
    integer,  intent(in)  ::   latndx(pcols)               ! lat  indicies in chunk
    ...
    real(r8)    :: frac
    real(r8)    :: zlev_top, zlev_bot, zdel, zdeli
    real(r8)    :: ztab_top, ztab_bot
    real(r8) :: srcs_offline( ncol, pver )
    integer :: ndx

    real(r8), dimension(ncol,pver) :: no_air, co_air, no_lgt
    ...
    airplane : if(has_airpl_src) then
             ...
             if( zlev_bot <= ztab_top .and. zlev_top >= ztab_bot ) then
                do kk = 1,nlev
                   if( zlev_bot <= air_altitude(kk+1) .and. zlev_top >= air_altitude(kk) ) then
                      ...
                      if( no_ndx > 0 ) then
                         extfrc(i,k,no_ndx) = extfrc(i,k,no_ndx) + frac * pno(ii,jj,kk) * zdeli
                         no_air(i,k) = frac * pno(ii,jj,kk) * zdeli
                      end if
                      if( xno_ndx > 0 ) then
                         extfrc(i,k,xno_ndx) = extfrc(i,k,xno_ndx) + frac * pno(ii,jj,kk) * zdeli
                      end if
                      if( co_ndx > 0 ) then
                         extfrc(i,k,co_ndx) = extfrc(i,k,co_ndx) + frac * pco(ii,jj,kk) * zdeli
                         co_air(i,k) = frac * pco(ii,jj,kk) * zdeli
                      end if
                   end if
                end do
             end if
    ...
    do i = 1,num_tracer_srcs
       ...
       call get_srcs_data( tracer_src_flds(i), srcs_offline,  ncol, lchnk )
       do k = 1,pver
