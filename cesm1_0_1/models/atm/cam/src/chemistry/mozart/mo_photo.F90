module mo_photo
  ...
  use mo_constants, only : pi,r2d,boltz=>boltzmann,d2r
...
contains
  ...
  subroutine photo_inti( xs_coef_file, xs_short_file, xs_long_file, rsf_file, &
    ...
    use mo_regrider,   only : regrid_inti, regrid_1d
...
#ifdef SPMD
    use mpishorthand,  only : mpicom, mpiint, mpir8, mpilog
#endif
...
!----------------------------------------------------------------------
      ...
      if( any( pht_alias_lst(:,1) /= ' ' ) ) then
         ...
         do ndx = 1,phtcnt
            if( pht_alias_lst(ndx,1) /= ' ' ) then
               if( pht_alias_mult(ndx,1) == 1. ) then
    ...
    Masterproc_only : if( masterproc ) then
       ...
       has_abs_columns : if( has_o2_col .or. has_o3_col ) then
          ...
          !       ... timing
          !-----------------------------------------------------------------------
          call wrap_inq_dimid( ncid, 'month', dimid )
          call wrap_inq_dimlen( ncid, dimid, ntimes )

          if( ntimes /= 12 ) then
             write(iulog,*) 'photo_inti: exo coldens is not annual period'
             call endrun
          end if
          ...
          call wrap_get_var_realx( ncid, vid, lats )
          ...
          call wrap_get_var_realx( ncid, vid, levs )
          ...
          if( astat /= 0 ) then
             write(iulog,*) 'photo_inti: failed to deallocate lats; error = ',astat
             call endrun
          end if
          jl = 1
          ju = plat
          jlim = (/ 1,nlat /)
          allocate( coldens(nlat,nlev),stat=astat )
          if( astat /= 0 ) then
             write(iulog,*) 'photo_inti: coldens allocation error = ',astat
             call endrun
          end if
          if( has_o2_col ) then
             allocate( o2_exo_coldens(nlev,plat,ntimes),stat=astat )
             if( astat /= 0 ) then
          ...
          if( has_o3_col ) then
             allocate( o3_exo_coldens(nlev,plat,ntimes),stat=astat )
             if( astat /= 0 ) then
          ...
          !	... read exo coldens
          !-----------------------------------------------------------------------
          allocate(wrk(plat),stat=astat)
          do n = 1,ntimes
             if( has_o2_col ) then
                call wrap_inq_varid( ncid, 'O2_column_density', vid )
                call wrap_get_vara_realx( ncid, vid, (/ 1, 1, n /), (/ nlat, nlev, 1 /), coldens )
                !-----------------------------------------------------------------------
                !	... regrid exo coldens
                !-----------------------------------------------------------------------
                do k = 1,nlev
             ...
             if( has_o3_col ) then
                call wrap_inq_varid( ncid, 'O3_column_density', vid )
                call wrap_get_vara_realx( ncid, vid, (/ 1, 1, n /), (/ nlat, nlev, 1 /), coldens )
                !-----------------------------------------------------------------------
                !	... regrid exo coldens
                !-----------------------------------------------------------------------
    ...
    if( has_o2_col .or. has_o3_col ) then
       ...
       call mpibcast( nlev,   1, mpiint, 0, mpicom )
       call mpibcast( ntimes, 1, mpiint, 0, mpicom )
       call mpibcast( ki,     1, mpiint, 0, mpicom ) 
       call mpibcast( delp,   1, mpir8, 0, mpicom )
       call mpibcast( has_fixed_press,   1, mpilog, 0, mpicom )
       ...
       if (.not. masterproc) then 
          ...
          allocate( days(ntimes),stat=astat )
          if( astat /= 0 ) then
             write(iulog,*) 'photo_inti: days allocation error = ',astat
             call endrun
          end if
       endif
       call mpibcast( levs, nlev, mpir8,  0, mpicom )
       call mpibcast( days, ntimes, mpir8,  0, mpicom )
       ...
       if( has_o2_col ) then
          ...
          call mpibcast( o2_exo_coldens, nlev*plat*ntimes, mpir8,  0, mpicom )
       endif
       if( has_o3_col ) then
          if (.not. masterproc) then 
             allocate( o3_exo_coldens(nlev,plat,ntimes),stat=astat )
          ...
          call mpibcast( o3_exo_coldens, nlev*plat*ntimes, mpir8,  0, mpicom )
       endif
    endif
#endif
...
!-----------------------------------------------------------------
    ...
    real(r8) :: o_den(pver)               ! o density (molecules/cm^3)
    real(r8) :: o2_den(pver)              ! o2 density (molecules/cm^3)
    real(r8) :: o3_den(pver)              ! o3 density (molecules/cm^3)
    real(r8) :: no_den(pver)              ! no density (molecules/cm^3)
    real(r8) :: n2_den(pver)              ! n2 density (molecules/cm^3)
    real(r8) :: jno_sht(pver)             ! no short photorate
    real(r8) :: jo2_sht(pver,2)        ! o2 short photorate
...
!-----------------------------------------------------------------
    if ( waccm ) then
       if (nsht>0) then
          allocate( sht_prates(pver,nsht),stat=astat )
          if( astat /= 0 ) then
...
!-----------------------------------------------------------------
    ...
    col_loop : do i = 1,ncol
       ...
       daylight : if( sza >= 0._r8 .and. sza < max_zen_angle ) then
          ...
          if ( waccm ) then
             ...
             if ( o_is_inv ) then
                o_den(:) = invariants(i,:,o_ndx)
             else
                o_den(:) = vmr(i,:,o_ndx) * invariants(i,:,indexm)
             endif
             if ( o2_is_inv ) then
                o2_den(:) = invariants(i,:,o2_ndx)
             else
                o2_den(:) = vmr(i,:,o2_ndx) * invariants(i,:,indexm)
             endif
             if ( o3_is_inv ) then
                o3_den(:) = invariants(i,:,o3_ndx)
             else
                o3_den(:) = vmr(i,:,o3_ndx) * invariants(i,:,indexm)
             endif
             if ( n2_is_inv ) then
                n2_den(:) = invariants(i,:,n2_ndx)
             else
                n2_den(:) = vmr(i,:,n2_ndx) * invariants(i,:,indexm)
             endif
             if ( no_is_inv ) then
                no_den(:) = invariants(i,:,no_ndx)
             else
                no_den(:) = vmr(i,:,no_ndx) * invariants(i,:,indexm)
             endif

             !-----------------------------------------------------------------
             !	... short wave length component
             !-----------------------------------------------------------------
             call jshort( pver, sza, n2_den, o2_den, o3_den, &
                          no_den, tline, zarg, jo2_sht, jno_sht, sht_prates )  
             do m = 1,phtcnt
                if( sht_indexer(m) > 0 ) then
                   alias_factor = pht_alias_mult(m,1)
                   if( alias_factor == 1. ) then
                      photos(i,pver:1:-1,m) = sht_prates(:,sht_indexer(m))
                   else
                      photos(i,pver:1:-1,m) = alias_factor * sht_prates(:,sht_indexer(m))
                   end if
                end if
             end do
             photos(i,pver:1:-1,jno_ndx)   = jno_sht(:)
             photos(i,pver:1:-1,jo2_a_ndx) = jo2_sht(:,2)
             photos(i,pver:1:-1,jo2_b_ndx) = jo2_sht(:,1)
  ...
  subroutine xactive_photo( photos, vmr, temper, cwat, cldfr, &
    ...
    !-----------------------------------------------------------------
    integer,  intent(in)    :: ncol
    integer,  intent(in)    :: lonndx(pcols),latndx(pcols)  ! grid index arrays
    real(r8), intent(in)    :: esfact                       ! earth sun distance factor
...
secant_in_bounds : &
          if( secant <= secant_limit ) then
             ...
             aersal(1,:) = aersal(2,:)
             call photoin( lat, ncdate, alat, along, &
...
#ifdef DEBUG
             write(iulog,*) 'xactive_photo: prates at lat,lon = ',lat,lon
#endif
               do m = 1,phtcnt
                  if( lng_indexer(m) > 0 ) then
                     alias_factor = pht_alias_mult(m,2)
...
!-----------------------------------------------------------------
  ...
  subroutine set_ub_col( col_delta, vmr, invariants, ps, pdel, ncol, latndx )
    !---------------------------------------------------------------
    ...
    has_abs_cols : if( has_o2_col .and. has_o3_col ) then
       if( has_fixed_press ) then
          ...
          if( has_o2_col ) then
             do i = 1,ncol
                ...
                if ( kl > 0 ) then
                   ...
                   tint_vals(1) = o2_exo_coldens( 1,lat,last) 
                   tint_vals(2) = o2_exo_coldens( 1,lat,next) 
                endif
          ...
          if( has_o3_col ) then
             do i = 1,ncol
                ...
                if ( kl > 0 ) then
                   ...
                   tint_vals(1) = o3_exo_coldens( 1,lat,last) 
                   tint_vals(2) = o3_exo_coldens( 1,lat,next) 
                endif
          ...
          !---------------------------------------------------------------
          call p_interp( latndx, ncol, ps, o2_exo_col, o3_exo_col )
       end if
    else
       o2_exo_col(:) = 0._r8
       o3_exo_col(:) = 0._r8
  ...
  subroutine p_interp( latndx, ncol, ps, o2_exo_col, o3_exo_col )
    !---------------------------------------------------------------
    ...
    integer, intent(in)     :: latndx(pcols)             ! latitude  indicies in chunk
    real(r8)                :: ps(pcols)                 ! surface pressure (Pa)
    ...
    do i = 1,ncol
       lat = latndx(i)
       pinterp = hyai(1) * ps0 + hybi(1)*ps(i)
       if( pinterp < levs(1) ) then
          ki   = 0
          delp = 0._r8
       else
