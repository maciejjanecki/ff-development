module mo_drydep
  ...
  public :: n_land_type, fraction_landuse
...
contains
  ...
  subroutine drydep_fromlnd( ocnfrac, icefrac, ncdate, sfc_temp, pressure_sfc,  &
    ...
    call drydep_xactive( ncdate, sfc_temp, pressure_sfc,  &
                         ...
                         tv, soilw, rh, ncol, lonndx, latndx, lchnk, beglandtype=7, endlandtype=8 )
    ...
    species_loop3 : do ispec = 1,nddvels
       ...
       !-------------------------------------------------------------------------------------
       dvel(:ncol,spc_ndx(ispec)) = lnd(lchnk)%dvel(:ncol,ispec) * lndfrac(:ncol) &
                                  + ocnice_dvel(:ncol,spc_ndx(ispec))*(ocnfrac(:ncol)+icefrac(:ncol))

       !-------------------------------------------------------------------------------------
       !        ... special adjustments
       !-------------------------------------------------------------------------------------
       if( spc_ndx(ispec) == mpan_ndx .or. spc_ndx(ispec) == xmpan_ndx ) then
          dvel(:ncol,spc_ndx(ispec)) = dvel(:ncol,spc_ndx(ispec))/3._r8
       endif

       !-------------------------------------------------------------------------------------
       !        ... compute the deposition flux
       !-------------------------------------------------------------------------------------
       dflx(:ncol,spc_ndx(ispec)) = dvel(:ncol,spc_ndx(ispec)) * term(:ncol) * mmr(:ncol,plev,spc_ndx(ispec))
  ...
  subroutine dvel_inti_table( depvel_file )
    ...
    !---------------------------------------------------------------------------
    if(masterproc) then
       write(iulog,*) 'dvel_inti: source lats (deg)'
       write(iulog,'(10F8.2)') dvel_lats
       write(iulog,*) 'dvel_inti: target lats (deg)'
       write(iulog,'(10F8.2)') clat*r2d
       write(iulog,*) 'dvel_inti: source lons (deg)'
       write(iulog,'(10F8.2)') dvel_lons
       write(iulog,*) 'dvel_inti: target lons (deg)'
       write(iulog,'(10F8.2)') clon*r2d
    end if
    ...
    if(masterproc) write(iulog,*) 'dvel_inti: REGRID_INTI using grid number ',gndx
    jl = 1
    ju = plat
    if( gndx /= 0 ) then
    ...
    if(masterproc) write(iulog,*) 'dvel_inti: jlim',jlim(:)
    ...
    !---------------------------------------------------------------------------
    allocate( dvel(plon,plat,nspecies,nmonth), stat=ierr )
    if( ierr /= 0 ) then
    ...
    do ispecies = 1,nspecies
       do m = 1,12
          call regrid_2d( dvel_in( :,jlim(1):jlim(2),ispecies,m ), dvel(:,:,ispecies,m), gndx, jl, ju, &
    ...
    !---------------------------------------------------------------------------
    allocate( dvel_interp(plon,plat,nspecies),stat=ierr )
    if( ierr /= 0 ) then
  ...
  subroutine drydep_table( calday, tsurf, zen_angle, &
                           depvel, dflx, q, p, &
                           tv, ncol, icefrac, ocnfrac, lonndx, latndx )
    !--------------------------------------------------------
    ...
    !           (also seaice and npp)
    !--------------------------------------------------------
    call interpdvel( calday )

    if( o3_in_tab ) then
       do i=1,ncol
          o3_tab_dvel(i) = dvel_interp(lonndx(i),latndx(i),o3_tab_ndx)
       enddo
    end if
    ...
    do m = 1,gas_pcnst
       if( map(m) /= 0 ) then
          do i = 1,ncol
             depvel(i,m) = dvel_interp(lonndx(i),latndx(i),map(m))
    ...
    if( .not. h2o2_in_tab ) then
       ...
       do i=1,ncol
          tmp(i) = dvel_interp(lonndx(i),latndx(i),h2o2_tab_ndx)
       enddo
    end if
    if( h2o2_dd ) then
    ...
    if( ch3ooh_in_tab ) then
       do i=1,ncol
          tmp(i) = dvel_interp(lonndx(i),latndx(i),ch3ooh_tab_ndx)
       enddo
    else
    ...
    if( co_in_tab ) then
       do i=1,ncol
          tmp(i) = dvel_interp(lonndx(i),latndx(i),co_tab_ndx)
       enddo
    else
       tmp(:) = 0._r8
    end if
    ...
    if( glyald_dd ) then
       if( map(glyald_ndx) == 0 ) then
          if( ch3cho_dd ) then
             ...
             do i=1,ncol
                depvel(i,glyald_ndx) = dvel_interp(lonndx(i),latndx(i),ch3cho_tab_ndx)
             enddo
          else
  ...
  subroutine dvel_inti_xactive( depvel_lnd_file, clim_soilw_file, season_wes_file )
    ...
    if(dycore_is('UNSTRUCTURED') ) then
       call endrun( 'This code is not supported for unstructured atmosphere grids '// &
            'use drydep_method=table in the drydep_inparm namelist')
    end if
    if( masterproc ) then
       write(iulog,*) 'drydep_inti: following species have dry deposition'
       do i=1,nddvels
    ...
    call pio_closefile( piofile )
    if(dycore_is('UNSTRUCTURED') ) then
       allocate(clat(plat*plon))
       call get_horiz_grid_d(plat*plon, clat_d_out=clat)
       jl = 1
       ju = plat*plon
    else
       allocate(clat(plat))
    ...
    deallocate( lat_lai, wk_lai, clat )
  ...
  subroutine interp_map( plon, plat, nlon_veg, nlat_veg, npft_veg, lat_veg, lat_veg_edge, &
    ...
    if(dycore_is('UNSTRUCTURED') ) then
       allocate( phi(plon*plat), lam(plon*plat), garea(plon*plat))
       call get_horiz_grid_d(plon*plat, area_d_out=garea, clon_d_out=lam, clat_d_out=phi)       
    else
  ...
  subroutine drydep_xactive( ncdate, sfc_temp, pressure_sfc,  &
                             wind_speed, spec_hum, air_temp, pressure_10m, rain, &
                             snow, solar_flux, dvel, dflx, mmr, &
                             tv, soilw, rh, ncol, lonndx, latndx, lchnk, beglandtype, endlandtype )
    !-------------------------------------------------------------------------------------
  ...
  !-------------------------------------------------------------------------------------
  !-------------------------------------------------------------------------------------
  subroutine set_soilw( soilw, ncol , lonndx, latndx, calday )
    !--------------------------------------------------------------------
