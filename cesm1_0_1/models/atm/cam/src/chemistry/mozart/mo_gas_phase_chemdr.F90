...
contains
  ...
  subroutine gas_phase_chemdr_inti()
    ...
    call addfld( 'SAD_SNAT', '/cm ', pver, 'I', 'small nat sad', phys_decomp )
    ...
    call addfld( 'RAD_SNAT', 'cm ', pver, 'I', 'small nat radius', phys_decomp )
    call addfld( 'RAD_LNAT', 'cm ', pver, 'I', 'large nat radius', phys_decomp )
    call addfld( 'RAD_ICE', 'cm ', pver, 'I', 'sad ice', phys_decomp )

    call addfld( 'HNO3_CD1', 'mol/mol', pver, 'I', 'STS condensed HNO3', phys_decomp )
    call addfld( 'HNO3_CD2', 'mol/mol', pver, 'I', 'condensed hno3 concentration', phys_decomp )
    call addfld( 'HNO3_CD3', 'mol/mol', pver, 'I', 'large-mode NAT condensed HNO3', phys_decomp )
...
!
    ...
    real(r8),       intent(inout) :: mmrcw(pcols,pver,gas_pcnst)    ! cloud-borne aerosol
    ...
    real(r8) :: hno3_cond(ncol,pver,3)         ! hno3 condensed phase concentration (mol/mol)
    ...
    real(r8) :: radius_strat(ncol,pver,4)      ! radius of sulfate, nat, & ice ( cm )
    real(r8) :: sad_strat(ncol,pver,4)         ! surf area density of sulfate, nat, & ice ( cm^2/cm^3 )
...
#if (defined MODAL_AERO)
    call mmr2vmr( mmrcw, vmrcw, mbar, ncol )
#endif
    ...
    call set_fstrat_h2o( latndx, h2ovmr, pmid, troplev, calday, ncol )
    ...
    !-----------------------------------------------------------------------      
    call strato_sad_set( pmid, latndx, lonndx, strato_sad, ncol)
    ...
    if ( has_strato_chem ) then
       ...
       !        ... call SAD routine
       !-----------------------------------------------------------------------      
       call sad_strat_calc( lchnk, invariants(1,1,indexm), pmb, tfld, hno3_gas, &
       ...
       call outfld( 'SAD_SNAT', sad_strat(:,:,2), ncol, lchnk )
       call outfld( 'SAD_LNAT', sad_strat(:,:,3), ncol, lchnk )
       call outfld( 'SAD_ICE', sad_strat(:,:,4), ncol, lchnk )
       call outfld( 'RAD_SULFC', radius_strat(:,:,1), ncol, lchnk )
       call outfld( 'RAD_SNAT', radius_strat(:,:,2), ncol, lchnk )
       call outfld( 'RAD_LNAT', radius_strat(:,:,3), ncol, lchnk )
       call outfld( 'RAD_ICE', radius_strat(:,:,4), ncol, lchnk )
       ...
       !-----------------------------------------------------------------------      
       call ratecon_sfstrat( invariants(1,1,indexm), pmid, tfld, &
            radius_strat, sad_strat, sad_strat(1,1,2), &
            sad_strat(1,1,4), h2ovmr, vmr, reaction_rates, ncol )
    ...
    !-----------------------------------------------------------------------      
    call set_ub_col( col_delta, vmr, invariants, ps, pdel, ncol, latndx)
    ...
    if ( xactive_prates ) then
       ...
       !-----------------------------------------------------------------
       call table_photo( reaction_rates, pmid, pdel, tfld, zmid, &
    ...
    do i = 1,rxt_tag_cnt
       call outfld( tag_names(i), reaction_rates(:,:,rxt_tag_map(i)), ncol, lchnk )
    enddo
...
! do aerosol coagulation
   ...
   call t_stopf('modal_coag')
    ...
    if ( has_strato_chem ) then
       ...
       endwhere
       call strat_aer_settling( invariants(1,1,indexm), pmid, delt, zmid, tfld, &
            hno3_cond(1,1,3), settl_rad, ncol, lchnk, 2 )
#else
       call strat_aer_settling( invariants(1,1,indexm), pmid, delt, zmid, tfld, &
            hno3_cond(1,1,3), radius_strat(1,1,3), ncol, lchnk, 2 )
#endif
       ...
       call outfld( 'HNO3_CD1', hno3_cond, ncol ,lchnk )
       call outfld( 'HNO3_CD2', hno3_cond(1,1,2), ncol ,lchnk )
       call outfld( 'HNO3_CD3', hno3_cond(1,1,3), ncol ,lchnk )
...
!
...
#ifdef MODAL_AERO
    call vmr2mmr( vmrcw, mmrcw, mbar, ncol )
#endif
    ...
    if( .not. dyn_soilw .and. table_soilw ) then
       call set_soilw( soilw, ncol , lonndx, latndx, calday )
    end if
...
#if (defined MODAL_AERO)
    call chm_diags( lchnk, ncol, vmr(:ncol,:,:), mmr_new(:ncol,:,:), mmrcw(:ncol,:,:), &
