...
! ion chemistry
!   
    ion1_ndx  = get_rxt_ndx( 'ion1' )
    ion2_ndx  = get_rxt_ndx( 'ion2' )
    ion3_ndx  = get_rxt_ndx( 'ion3' )
    ion11_ndx  = get_rxt_ndx( 'ion11' )
    ...
    if (masterproc) then
       ...
       write(iulog,'(10i5)') usr_O_O2_ndx,usr_HO2_HO2_ndx,tag_NO2_NO3_ndx,usr_N2O5_M_ndx,tag_NO2_OH_ndx,usr_HNO3_OH_ndx &
                            ...
                            ,usr_CO_OH_a_ndx,tag_C2H4_OH_ndx,tag_C3H6_OH_ndx,tag_CH3CO3_NO2_ndx,usr_PAN_M_ndx,usr_CH3COCH3_OH_ndx &
  ...
  subroutine usrrxt( rxt, temp, tempi, tempe, invariants, h2ovmr,  ps, &
                     ...
                     ltrop, ncol, sad_total, cwat, mbar )
...
!-----------------------------------------------------------------
    ...
    real(r8) ::  v, rho_air, n, n_exp, r_rd, r_sd
...
!-----------------------------------------------------------------
       ...
       if( usr_XHO2NO2_M_ndx > 0 ) then
          if( tag_NO2_HO2_ndx > 0 ) then
             call comp_exp( exp_fac, -10900.*tinv, ncol )
...
!-----------------------------------------------------------------
       ...
       if( usr_XMPAN_M_ndx > 0 ) then
          if( usr_MCO3_NO2_ndx > 0 ) then
             rxt(:,k,usr_XMPAN_M_ndx) = rxt(:,k,usr_MCO3_NO2_ndx) * 1.111e28 * exp_fac(:)
          else
       ...
       if( usr_NO2_aer_ndx > 0 .or. usr_NO3_aer_ndx > 0 .or. usr_N2O5_aer_ndx > 0 .or. usr_HO2_aer_ndx > 0 ) then
          ...
          long_loop : do i = 1,ncol
             ...
             if( soa_ndx > 0 ) then
                ...
                n  = v * (6._r8/pi)*(1./(dm_orgc**3._r8))*n_exp
                s_exp     = exp(2._r8*log_sd_orgc*log_sd_orgc)
             ...
             if( usr_N2O5_aer_ndx > 0 ) then
                ...
                rxt_sulf = sfc_sulf / (0.5_r8*dm_sulf_wet/dg + (4._r8/(c_n2o5*gamma_n2o5))) 
                rxt_nit = sfc_nit / (0.5_r8*dm_sulf_wet/dg + (4._r8/(c_n2o5*gamma_n2o5))) 
                rxt_oc = sfc_oc / (0.5_r8*dm_orgc_wet/dg + (4._r8/(c_n2o5*gamma_n2o5)))
                rxt_soa = sfc_soa / (0.5_r8*dm_orgc_wet/dg + (4._r8/(c_n2o5*gamma_n2o5)))

                rxt(i,k,usr_N2O5_aer_ndx) = rxt_sulf + rxt_nit + rxt_oc + rxt_soa
             ...
             if( usr_XNO2NO3_aer_ndx > 0 ) then
                ...
                rxt(i,k,usr_XNO2NO3_aer_ndx) = rxt_sulf + rxt_nit + rxt_oc + rxt_soa
             ...
             if( usr_NO2XNO3_aer_ndx > 0 ) then
                ...
                rxt(i,k,usr_NO2XNO3_aer_ndx) = rxt_sulf + rxt_nit + rxt_oc + rxt_soa
             ...
             if( usr_NO3_aer_ndx > 0 ) then
                ...
                rxt_sulf = sfc_sulf / (0.5_r8*dm_sulf_wet/dg + (4._r8/(c_no3*gamma_no3))) 
                rxt_nit = sfc_nit / (0.5_r8*dm_sulf_wet/dg + (4._r8/(c_no3*gamma_no3))) 
                rxt_oc = sfc_oc / (0.5_r8*dm_orgc_wet/dg + (4._r8/(c_no3*gamma_no3)))
                rxt_soa = sfc_soa / (0.5_r8*dm_orgc_wet/dg + (4._r8/(c_no3*gamma_no3)))

                rxt(i,k,usr_NO3_aer_ndx) = rxt_sulf + rxt_nit + rxt_oc + rxt_soa
             end if
             if( usr_XNO3_aer_ndx > 0 ) then
                ...
                rxt(i,k,usr_XNO3_aer_ndx) = rxt_sulf + rxt_nit + rxt_oc + rxt_soa
             end if
             !-------------------------------------------------------------------------
             ...
             if( usr_NO2_aer_ndx > 0 ) then
                ...
                rxt_sulf = sfc_sulf / (0.5_r8*dm_sulf_wet/dg + (4._r8/(c_no2*gamma_no2))) 
                rxt_nit = sfc_nit / (0.5_r8*dm_sulf_wet/dg + (4._r8/(c_no2*gamma_no2))) 
                rxt_oc = sfc_oc / (0.5_r8*dm_orgc_wet/dg + (4._r8/(c_no2*gamma_no2)))
                rxt_soa = sfc_soa / (0.5_r8*dm_orgc_wet/dg + (4._r8/(c_no2*gamma_no2)))

                rxt(i,k,usr_NO2_aer_ndx) = rxt_sulf + rxt_nit + rxt_oc + rxt_soa
             end if
             if( usr_XNO2_aer_ndx > 0 ) then
                ...
                rxt(i,k,usr_XNO2_aer_ndx) = rxt_sulf + rxt_nit + rxt_oc + rxt_soa
             end if
             !-------------------------------------------------------------------------
             ...
             if( usr_HO2_aer_ndx > 0 ) then
                ...
                rxt_sulf = sfc_sulf / (0.5_r8*dm_sulf_wet/dg + (4._r8/(c_ho2*gamma_ho2))) 
                rxt_nit = sfc_nit / (0.5_r8*dm_sulf_wet/dg + (4._r8/(c_ho2*gamma_ho2))) 
                rxt_oc = sfc_oc / (0.5_r8*dm_orgc_wet/dg + (4._r8/(c_ho2*gamma_ho2)))
                rxt_soa = sfc_soa / (0.5_r8*dm_orgc_wet/dg + (4._r8/(c_ho2*gamma_ho2)))

                rxt(i,k,usr_HO2_aer_ndx) = rxt_sulf + rxt_nit + rxt_oc + rxt_soa
             end if
          end do long_loop
       end if
