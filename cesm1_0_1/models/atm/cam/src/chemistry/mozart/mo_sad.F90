      ...
      use phys_buffer,   only : pbuf_add, pbuf
    ...
    contains
      ...
      subroutine sad_setopts( strat_aero_feedback_in )
        ...
        if ( rad_feedback ) then
           call pbuf_add('H2SO4M' , 'global', 1,pver,1, h2so4_ndx) 
        endif
...
!---------------------------------------------------------------------- 
	 ...
	 if( (hyam(k) + hybm(k)) < .001_r8 ) then
      ...
      write(iulog,*) '          whose midpoint is ',(hyam(sad_topp)+hybm(sad_topp))*1.e3_r8,' hPa'

      if (rad_feedback .and. is_first_step()) then
         pbuf(h2so4_ndx)%fld_ptr = 0.0_r8
      endif
...
!-------------------------------------------------------------------------------
      ...
      integer, intent(in)     ::  latndx(pcols)              ! global lat indicies
      integer, intent(in)     ::  lonndx(pcols)              ! global lon indicies
      ...
      real(r8), intent(inout) ::  hno3_cond(ncol,pver,3)     ! HNO3 condensed phase (mole fraction)
      real(r8), intent(out)   ::  radius_strat(ncol,pver,4)  ! Radius of Sulfate, NAT_sm, NAT_lg, and ICE (cm)
      real(r8), intent(out)   ::  sad_strat(ncol,pver,4)     ! Surface area density of Sulfate, NAT_sm, NAT_lg, ICE (cm2 cm-3)
...
!-------------------------------------------------------------------------------
      ...
      dens_fctr = 1.e-11 * boltz * avogad / mwdry

      h2so4m => pbuf(h2so4_ndx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
...
!----------------------------------------------------------------------
	    ...
	    if( any(mask(:,k)) ) then
         ...
         call sulfate_sad_calc( ncol, press, temp, h2o_avail, hno3_avail, &
                                sad_sage, m, hno3_gas_sulf, hno3_cond_sulf, sad_sulfate, &
                                radius_sulfate, mask, lchnk, 1, h2so4m(:ncol,:), .false. )
         do k = sad_topp,pver
            where( mask(:,k) )
               sad_strat(:,k,1)    = sad_sulfate(:,k)
               sad_strat(:,k,2)    = 0.0_r8
...
!----------------------------------------------------------------------
	    ...
	    if( any(mask1(:,k)) ) then
         ...
         call sulfate_sad_calc( ncol, press, temp, h2o_avail, hno3_avail, &
                                sad_sage, m, hno3_gas_sulf, hno3_cond_sulf, sad_sulfate, &
                                radius_sulfate, mask1, lchnk, 2, h2so4m(:ncol,:), .false. )
         do k = sad_topp,pver
            where( mask1(:,k) )
               sad_strat(:,k,1)    = sad_sulfate(:,k)
               sad_strat(:,k,2)    = 0._r8
...
sulfate_sad : &
      if( any( mask(:,sad_topp:pver) ) ) then
	    ...
	    if( any(mask(:,k)) ) then
         ...
         call sulfate_sad_calc( ncol, press, temp, h2o_avail, hno3_avail, &
                                sad_sage, m, hno3_gas_sulf, hno3_cond_sulf, sad_sulfate, &
                                radius_sulfate, mask, lchnk, 3, h2so4m(:ncol,:), .false. )
         do k = sad_topp,pver
            where( mask(:,k) )
               sad_strat(:,k,1)    = sad_sulfate(:,k)
               sad_strat(:,k,2)    = 0._r8
      ...
      ! need mmr for radiation
      ! micrograms/m^3 --> kg/kg
