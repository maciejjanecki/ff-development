...
contains
  ...
  subroutine extfrc_inti( extfrc_specifier, extfrc_type, extfrc_ymd, extfrc_tod )
    ...
    integer  , intent(in)        :: extfrc_ymd
    integer  , intent(in)        :: extfrc_tod
    ...
    if (masterproc) then
       ...
       if( extfrc_type == 'SERIAL' ) then
          write(iulog,*) ' start date = ', extfrc_ymd
          write(iulog,*) ' start time = ', extfrc_tod
       else if( extfrc_type == 'CYCLICAL' ) then
          write(iulog,*) ' cycle year = ',extfrc_ymd/10000
       end if
       write(iulog,*) ' '
    ...
    frcing_loop: do m = 1, extfrc_cnt
       ...
       call trcdata_init( forcings(m)%sectors, &
                          ...
                          forcings(m)%fields,  &
                          forcings(m)%file, &
                          rmv_file, extfrc_ymd, extfrc_tod, extfrc_type )
  ...
  subroutine extfrc_timestep_init( state )
    !-----------------------------------------------------------------------
    ...
    do m = 1,extfrc_cnt
       call advance_trcdata( forcings(m)%fields, forcings(m)%file, state  )
    end do
