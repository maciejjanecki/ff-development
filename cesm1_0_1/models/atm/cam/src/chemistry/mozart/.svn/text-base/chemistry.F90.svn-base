...
!---------------------------------------------------------------------------------
  ...
  character(len=256) :: bndtvg = ' ' ! pathname for greenhouse gas loss rate
  character(len=256) :: h2orates = ' ' ! pathname for greenhouse gas (lyman-alpha H2O loss)
  ...
  character(len=256) :: rsf_file = 'rsf_file'
  character(len=256) :: exo_coldens_file = ''
  character(len=256) :: tuv_xsect_file = 'tuv_xsect_file'
  character(len=256) :: o2_xsect_file = 'o2_xsect_file'
  character(len=256) :: xs_coef_file = 'xs_coef_file'
  character(len=256) :: xs_short_file = 'xs_short_file'
  character(len=256) :: xs_long_file = 'xs_long_file'
  character(len=256) :: electron_file = 'electron_file'
  character(len=256) :: euvac_file = 'euvac_file'
  character(len=256) :: euvacdat_file = 'euvacdat_file'

  ! solar / geomag data

  character(len=256) :: solar_parms_file = ' '     ! solar variability parameters
  character(len=256) :: photon_file = 'photon_file'

  ! dry dep
  
  character(len=256) :: depvel_file = 'depvel_file'
  character(len=256) :: depvel_lnd_file = 'depvel_lnd_file'
  character(len=256) :: clim_soilw_file = 'clim_soilw_file'
  character(len=256) :: season_wes_file = 'season_wes_file'

  ! emis

  character(len=256) :: airpl_emis_file = '' ! airplane emissions
  character(len=256) :: srf_emis_specifier(pcnst) = ''
  character(len=256) :: ext_frc_specifier(pcnst) = ''

  character(len=24)  :: srf_emis_type = 'CYCLICAL' ! 'CYCLICAL' | 'SERIAL' |  'INTERP_MISSING_MONTHS'
  integer            :: srf_emis_ymd  = 0          ! yyyymmdd - start date emis data (default is current model date)
  integer            :: srf_emis_tod  = 0          ! start time of day (seconds) of the emis data (default is current model tim

  character(len=24)  :: ext_frc_type = 'CYCLICAL' ! 'CYCLICAL' | 'SERIAL' |  'INTERP_MISSING_MONTHS'
  integer            :: ext_frc_ymd  = 0          ! yyyymmdd - start date emis data (default is current model date)
  integer            :: ext_frc_tod  = 0          ! start time of day (seconds) of the emis data (default is current model tim

  ! fixed lower boundary 

  character(len=256) :: flbc_file = 'flbc_file'
  character(len=16)  :: flbc_list(pcnst) = ''

  ! fixed stratosphere
  
  character(len=256) :: fstrat_file = 'fstrat_file'
  character(len=16)  :: fstrat_list(pcnst)  = ''

  ! stratospheric aerosols

  character(len=256) :: sad_file = 'sad_file'

  ! trop sulf

  character(len=256) :: sulf_file = 'sulf_file'
...
!---------------------------------------------------------------------------------
  ...
  character(len=16), parameter :: chem_name = 'waccm_mozart'
#elif ( defined WACCM_GHG )
  character(len=16), parameter :: chem_name = 'waccm_ghg'
#elif( defined TROPCHEM )
#if( defined SUPERFASTLLNL )
  character(len=16), parameter :: chem_name = 'super_fast_llnl'
#else
  character(len=16), parameter :: chem_name = 'trop_mozart'
#endif
#endif
...
logical function chem_is (name)
   ...
   if (trim(name) == trim(chem_name)) then
      chem_is = .true.
   end if
...
!-----------------------------------------------------------------------
    ...
    character(len=256) :: locfn                     ! chemistry data filespec
...
!================================================================================================
  ...
  subroutine chem_readnl(nlfile)
   ...
   ! args
    ...
    integer            :: flbc_date = 19900101       ! yyyymmdd for SERIAL(yyyy) or FIXED 
    integer            :: flbc_yr_offset = 0         ! yrs added to current yr for CYCLICAL

    character(len=8)   :: sad_type = 'CYCLICAL'      ! 'CYCLICAL' | 'SERIAL' | 'FIXED'
    integer            :: sad_date = 19900101        ! yyyymmdd for SERIAL(yyyy) or FIXED 
    integer            :: sad_yr_offset = 0          ! yrs added to current yr for CYCLICAL

    logical            :: use_cam_sulfchem   
    character(len=16), dimension(pcnst) :: aer_wetdep_list = ' '

    ! linoz data
    character(len=256) :: linoz_data_file               ! prescribed data file
    character(len=256) :: linoz_data_filelist           ! list of prescribed data files (series of files)
    character(len=256) :: linoz_data_path               ! absolute path of prescribed data files 
    ...
    integer            :: linoz_data_ymd                ! yyyymmdd - start date of the prescribed data (default is current model date)
    integer            :: linoz_data_tod                ! start time of day (seconds) of the prescribed data (default is current model time)
    ...
    character(len=256) :: tracer_cnst_specifier(MAXTRCRS) ! string array where each 
    ...
    character(len=256) :: tracer_srcs_specifier(MAXTRCRS) ! string array where each 
    ...
    integer            :: tgcm_ubc_ymd
    integer            :: tgcm_ubc_tod
    ...
    namelist /chem_inparm/ chem_freq, airpl_emis_file, &
         ...
         sad_type, sad_date, sad_yr_offset, sad_file, &
         ...
         srf_emis_type, srf_emis_ymd, srf_emis_tod, srf_emis_specifier,  &
         flbc_type, flbc_date, flbc_yr_offset, flbc_list, flbc_file, &
         fstrat_file, fstrat_list, &
         ext_frc_specifier, ext_frc_type, ext_frc_ymd, ext_frc_tod, &
    ...
    namelist /chem_inparm/ &
         ...
         linoz_data_type, &
         linoz_data_rmfile, linoz_data_ymd, linoz_data_tod
    namelist /chem_inparm/ &
         chlorine_loading_file, chlorine_loading_type, chlorine_loading_ymd, chlorine_loading_tod
    ...
    namelist /chem_inparm/ &
         ...
         tracer_cnst_rmfile, tracer_cnst_ymd, tracer_cnst_tod, &
         tracer_srcs_rmfile, tracer_srcs_ymd, tracer_srcs_tod 

#if ( defined WACCM_MOZART || defined WACCM_GHG )
    ! upper boundary conditions
    namelist /chem_inparm/ tgcm_ubc_file, tgcm_ubc_data_type, tgcm_ubc_ymd, tgcm_ubc_tod, &
                           snoe_ubc_file, t_pert_ubc, no_xfac_ubc
#endif

#ifdef WACCM_MOZART
    ! waccm solar proton namelist variables
    namelist /chem_inparm/ spe_data_file, spe_remove_file, spe_filenames_list, spe_restart
#endif
    ...
    call linoz_data_defaultopts( &
         ...
         linoz_data_ymd_out       = linoz_data_ymd,       &
         linoz_data_tod_out       = linoz_data_tod        ) 
    call tracer_cnst_defaultopts( &
         ...
         tracer_cnst_ymd_out       = tracer_cnst_ymd,       &
         tracer_cnst_tod_out       = tracer_cnst_tod        ) 
    call tracer_srcs_defaultopts( &
         ...
         tracer_srcs_ymd_out       = tracer_srcs_ymd,       &
         tracer_srcs_tod_out       = tracer_srcs_tod        )
    ...
    call ubc_defaultopts( &
         ...
         tgcm_ubc_ymd_out = tgcm_ubc_ymd, &
         tgcm_ubc_tod_out = tgcm_ubc_ymd )
#endif
    ...
    call mpibcast (srf_emis_ymd,      1,                               mpiint,  0, mpicom)
    call mpibcast (srf_emis_tod,      1,                               mpiint,  0, mpicom)
    ...
    call mpibcast (ext_frc_ymd,       1,                               mpiint,  0, mpicom)
    call mpibcast (ext_frc_tod,       1,                               mpiint,  0, mpicom)

    ! fixed lower boundary 

    call mpibcast (flbc_type,         len(flbc_type),                  mpichar, 0, mpicom)
    call mpibcast (flbc_date,         1,                               mpiint,  0, mpicom)
    call mpibcast (flbc_yr_offset,    1,                               mpiint,  0, mpicom)
    ...
    call mpibcast (sad_date,          1,                               mpiint,  0, mpicom)
    call mpibcast (sad_yr_offset,     1,                               mpiint,  0, mpicom)
    ...
    call mpibcast (tgcm_ubc_ymd, 1,                      mpiint,  0, mpicom)
    call mpibcast (tgcm_ubc_tod, 1,                      mpiint,  0, mpicom)
   ...
   call mpibcast (spe_restart        ,1                      ,mpilog, 0, mpicom )
   ...
   call mpibcast (strat_aero_feedback,1                      ,mpilog, 0, mpicom )
    ...
    call mpibcast (linoz_data_ymd,       1,                                     mpiint, 0,  mpicom)
    call mpibcast (linoz_data_tod,       1,                                     mpiint, 0,  mpicom)
    ...
    call mpibcast (chlorine_loading_ymd, 1,                          mpiint,  0, mpicom)
    call mpibcast (chlorine_loading_tod, 1,                          mpiint,  0, mpicom)
    ...
    call mpibcast (tracer_cnst_ymd,       1,                                      mpiint, 0,  mpicom)
    call mpibcast (tracer_cnst_tod,       1,                                      mpiint, 0,  mpicom)
    ...
    call mpibcast (tracer_srcs_ymd,       1,                                      mpiint,  0, mpicom)
    call mpibcast (tracer_srcs_tod,       1,                                      mpiint,  0, mpicom)

#endif

    flbc_timing%type      = flbc_type
    flbc_timing%date      = flbc_date
    flbc_timing%yr_offset = flbc_yr_offset

    sad_timing%type      = sad_type
    sad_timing%date      = sad_date
    sad_timing%yr_offset = sad_yr_offset
   ...
   call linoz_data_setopts( &
        ...
        linoz_data_ymd_in       = linoz_data_ymd,       &
        linoz_data_tod_in       = linoz_data_tod )
   call tracer_cnst_setopts( &
        ...
        tracer_cnst_ymd_in       = tracer_cnst_ymd,       &
        tracer_cnst_tod_in       = tracer_cnst_tod )
   call tracer_srcs_setopts( &
        ...
        tracer_srcs_ymd_in       = tracer_srcs_ymd,       &
        tracer_srcs_tod_in       = tracer_srcs_tod )
   ...
   call ubc_setopts( &
        ...
        tgcm_ubc_ymd_in = tgcm_ubc_ymd, &
        tgcm_ubc_tod_in = tgcm_ubc_ymd )
#endif
...
#if (defined MODAL_AERO)
    ndx_qqcw = pbuf_get_fld_idx( 'QQCW' )
#endif
...
!-----------------------------------------------------------------------
    call chemini &
     ...
     call chlorine_loading_init( chlorine_loading_file, &
                                 ...
                                 ymd = chlorine_loading_ymd, &
                                 tod = chlorine_loading_tod )
...
!-----------------------------------------------------------------------
...
#if (defined MODAL_AERO)
    qqcw => pbuf(ndx_qqcw)%fld_ptr(1,1:pcols,1:pver,lchnk,1:pcnst)
#endif
    ...
    call gas_phase_chemdr(lchnk, ncol, imozart, state%q, ptend%q, &
                          ...
                          cldw, tropLev, &
#if (defined MODAL_AERO)
                          ncldwtr, qqcw(1:pcols,1:pver,imozart:pcnst), pblh, &
#endif
...
!-------------------------------------------------------------------
  ...
  subroutine chem_init_restart( File )
    ...
    use spedata, only : init_spedata_restart
    implicit none
    ...
    call init_spedata_restart( File )
...
!-------------------------------------------------------------------
  subroutine chem_write_restart( File )
    ...
    use spedata,     only: write_spedata_restart
    use pio, only : file_desc_t
    implicit none
    ...
    call write_spedata_restart(File)
...
!-------------------------------------------------------------------
  subroutine chem_read_restart( File )
    ...
    use spedata,     only: read_spedata_restart
    ...
    call read_spedata_restart(File)
