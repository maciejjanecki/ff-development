...
module tracer_cnst
  ...
  character(len=16), pointer :: tracer_cnst_flds(:)
...
!-------------------------------------------------------------------
  subroutine tracer_cnst_setopts(       &
    ...
    integer,          intent(in), optional :: tracer_cnst_ymd_in
    integer,          intent(in), optional :: tracer_cnst_tod_in
    ...
    if ( present(tracer_cnst_ymd_in) ) then
       start_ymd = tracer_cnst_ymd_in
    endif
    if ( present(tracer_cnst_tod_in) ) then
       start_tod = tracer_cnst_tod_in
    endif
...
!-------------------------------------------------------------------
  subroutine tracer_cnst_defaultopts(   &
    ...
    integer,          intent(out), optional :: tracer_cnst_ymd_out
    integer,          intent(out), optional :: tracer_cnst_tod_out
    ...
    if ( present(tracer_cnst_ymd_out) ) then
       tracer_cnst_ymd_out = start_ymd
    endif
    if ( present(tracer_cnst_tod_out) ) then
       tracer_cnst_tod_out = start_tod
    endif
...
!-------------------------------------------------------------------
!-------------------------------------------------------------------
  subroutine tracer_cnst_adv( state )
    ...
    do i = 1,num_tracer_cnst
       ...
       do c = begchunk,endchunk
          ...
          case ('kg/kg','mmr')
             to_vmr(:ncol,:) = amass/fix_mass(ind)
          ...
          fields(i)%data(:ncol,:,c+fields(i)%chnk_offset) = to_vmr(:ncol,:) * fields(i)%data(:ncol,:,c+fields(i)%chnk_offset)      ! vmr
          call outfld( trim(tracer_cnst_flds(i)), fields(i)%data(:ncol,:,c+fields(i)%chnk_offset), ncol, state(c)%lchnk )
...
!-------------------------------------------------------------------
!-------------------------------------------------------------------
  subroutine get_cnst_data( field_name, data, ncol, lchnk  )
    ...
    call get_fld_data( fields, field_name, data, ncol, lchnk  )

  end subroutine get_cnst_data

!-------------------------------------------------------------------
!-------------------------------------------------------------------
  subroutine get_cnst_data_ptr(name, state, q)
    ...
    call get_fld_data( fields, name, data_q(:,:,idx,lchnk), ncol, lchnk  )

    data_q(:ncol,:,idx,lchnk) = data_q(:ncol,:,idx,lchnk)*fix_mass(inv_id)/amass   ! vmr->mmr
    q => data_q(:,:,idx,lchnk)
