...
! !PUBLIC MEMBERS
  ...
  public init_spedata_restart
  public write_spedata_restart
  public read_spedata_restart
...
!--------------------------------------------------------------------------
  subroutine spedata_defaultopts( spe_data_file_out, &
    ...
    logical, intent(out), optional :: spe_restart_out
    ...
    if ( present( spe_restart_out ) ) then
...
!--------------------------------------------------------------------------
  subroutine spedata_setopts( spe_data_file_in, &
    ...
    logical, intent(in), optional :: spe_restart_in
    integer :: ierr
    ...
    if ( present( spe_restart_in ) ) then
       spe_restart = spe_restart_in 
    endif
    ...
    if (masterproc) then
       ...
       if ( spe_run ) then
         ...
         write(iulog,*) 'Solar proton restart (branch) from previous SPE run: ', spe_restart
       endif
    endif
...
!--------------------------------------------------------------------------
  ...
  use time_manager,    only : get_calendar
...
!--------------------------------------------------------------------------
    integer :: astat
    integer :: ierr
    calendar = trim( to_upper(get_calendar()) )
...
!------------------------------------------------------------------------
  ...
  subroutine init_spedata_restart( piofile )
    implicit none
    type(file_desc_t), intent(inout) :: piofile
 
    character(len=32) :: name
    integer :: ioerr, mcdimid
    ...
    if(len_trim(curr_filename)>1) then
       allocate(currfnameid)
       name = 'spedata_curr_fname'
       ioerr = pio_def_var(pioFile, name,pio_char,  (/mcdimid/), currfnameid)
    else
       nullify(currfnameid)
    end if

    if(len_trim(next_filename)>1) then
       allocate(nextfnameid)
       name = 'spedata_next_fname'
       ioerr = pio_def_var(pioFile, name,pio_char,  (/mcdimid/), nextfnameid)
    else
       nullify(nextfnameid)
    end if
  end subroutine init_spedata_restart
!-------------------------------------------------------------------------
! writes file names to restart file - should probably use tracer_data?
!-------------------------------------------------------------------------
  subroutine write_spedata_restart( piofile )
    implicit none

    type(file_desc_t), intent(inout) :: piofile

    integer :: ioerr   ! error status
    if(associated(currfnameid)) then
       ioerr = pio_put_var(pioFile, currfnameid, curr_filename)
       deallocate(currfnameid)
       nullify(currfnameid)
    end if
    if(associated(nextfnameid)) then
       ioerr = pio_put_var(pioFile, nextfnameid, next_filename)
       deallocate(nextfnameid)
       nullify(nextfnameid)
    end if
  end subroutine write_spedata_restart

!-------------------------------------------------------------------------
! reads file names from restart file - should use tracer_data
!-------------------------------------------------------------------------
  subroutine read_spedata_restart( piofile )
    implicit none
    ...
    if(ioerr==PIO_NOERR) then
       curr_filename=' '
       ioerr = pio_get_var(piofile, vdesc, curr_filename)
    end if
    ...
    if(ioerr==PIO_NOERR) then
       next_filename=' '
       ioerr = pio_get_var(piofile, vdesc,next_filename)
    end if
    call PIO_SetErrorHandling(piofile, PIO_INTERNAL_ERROR)



  end subroutine read_spedata_restart
