...
! ===========================================
  ...
  subroutine global_dof(par,elem)
    ...
    ! ===================
#ifdef _STAGGER
    call initLongEdgeBuffer(edge,2)
#else
...
! redundent points of the array
...
#ifdef _STAGGER
     allocate(numElemV(nelem))
     allocate(numElem2V(nelem))
     numElemV=0;numElem2v=0
#endif
...
#ifdef _STAGGER
	numElemV(ig) = elem(ie)%idxV%NumUniquePts
#endif
     enddo
#ifdef _MPI
     call MPI_Allreduce(numElemP,numElem2P,nelem,MPIinteger_t,MPI_SUM,par%comm,ierr) 
#ifdef _STAGGER
     call MPI_Allreduce(numElemV,numElem2V,nelem,MPIinteger_t,MPI_SUM,par%comm,ierr) 
#endif
#else
     numElem2P=numElemP
#ifdef _STAGGER
...
#ifndef CAM
#if defined(PIO)
        elem(ie)%idxP%FileOffset=elem(ie)%idxP%UniquePtOffset
#else
	if(ie.eq.1) then
           elem(ie)%idxP%FileOffset=1
        else	
           elem(ie)%idxP%FileOffset=elem(ie-1)%idxP%FileOffset+elem(ie-1)%idxP%NumUniquePts
        end if
#endif
#endif
     enddo
     ...
     gOffset(1)=1
     do ig=2,nelem
	gOffset(ig) = gOffset(ig-1)+numElem2V(ig-1)
     enddo
    
     if(Debug) print *,'SetElemOffset: point #5'

     do ie=1,nelemd
        ig = elem(ie)%GlobalId
        elem(ie)%idxV%UniquePtOffset=gOffset(ig)
#ifndef CAM
#if defined(PIO)
        elem(ie)%idxV%FileOffset=elem(ie)%idxV%UniquePtOffset
#else
	if(ie.eq.1) then
           elem(ie)%idxV%FileOffset=1
        else	
           elem(ie)%idxV%FileOffset=elem(ie-1)%idxV%FileOffset+elem(ie-1)%idxV%NumUniquePts
        end if
#endif
#endif
     enddo
     GlobalUniqueColsV = gOffset(nelem)+numElem2V(nelem)-1
     deallocate(numElemV)
     deallocate(numElem2V)
#else
  ...
  subroutine CreateMetaData(par,elem,subelement_corners)
    type (parallel_t),intent(in) :: par
    type (element_t), target    :: elem(:)

    integer, intent(out)         :: subelement_corners((np-1)*(np-1)*nelemd,4)

    type (index_t), pointer  :: idx 
    type (LongEdgeBuffer_t)    :: edge
    integer :: i, j, ii, ie, base
    integer(kind=long_kind), pointer :: gdof(:,:)
    integer(kind=int_kind) :: fdofp(np,np,nelemd)
    ...
    do ie=1,nelemd
       ...
       do ii=1,idx%NumUniquePts
          ...
          fdofp(i,j,ie) = -(idx%UniquePtoffset+ii-1)
       end do
       call LongEdgeVpack(edge,fdofp(:,:,ie),1,0,elem(ie)%desc)
    end do
    call bndry_exchangeV(par,edge)
    do ie=1,nelemd
       base = (ie-1)*(np-1)*(np-1)
       call LongEdgeVunpackMIN(edge,fdofp(:,:,ie),1,0,elem(ie)%desc)
       ii=0
       do j=1,np-1
          do i=1,np-1
             ii=ii+1
             subelement_corners(base+ii,1) = -fdofp(i,j,ie)
             subelement_corners(base+ii,2) = -fdofp(i,j+1,ie)
             subelement_corners(base+ii,3) = -fdofp(i+1,j+1,ie)
             subelement_corners(base+ii,4) = -fdofp(i+1,j,ie)
          end do
       end do
    end do
