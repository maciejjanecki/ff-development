module prim_state_mod
  ...
  ! ------------------------------
  use dimensions_mod, only : nlev, nv, qsize_d, qsize
  ! ------------------------------
  ...
  use control_mod, only : integration, test_case, runtype, moisture, tracer_advection_formulation,&
       TRACERADV_TOTAL_DIVERGENCE,TRACERADV_UGRADQ,tstep_type,energy_fixer, qsplit, ftype, subcycle
  ! ------------------------------
  ...
  ! ------------------------------
  use reduction_mod, only : parallelsum,parallelmax,parallelmin
  ! ------------------------------
implicit none
private
...
!       write(10,*)time,c0
  ...
  subroutine prim_printstate(elem, tl,hybrid,hvcoord,nets,nete)
    ...
    real (kind=real_kind)  :: tmp(nv,nv,nets:nete)
    real (kind=real_kind)  :: tmp1(nets:nete)
    real (kind=real_kind)  :: ps(nv,nv)
    real (kind=real_kind)  :: dp(nv,nv)
    !    real (kind=real_kind)  :: E(nv,nv)
    ...
    real(kind=real_kind) :: v1, v2, vco(nv,nv,2,nlev)
    ...
    !   Mass                            time ( Mass-Q1mass(:,:) conserved )
    !   Q1Mass                          time  
    !   Qmass(:,:,4)                    time after robert 
    !   Qmass(:,:,3)                    time after forcing
    ...
    do q=1,qsize
       do ie=nets,nete
          tmp1(ie) = MINVAL(elem(ie)%state%Q(:,:,:,q,nm1))
       enddo
       qvmin_p(q) = ParallelMin(tmp1,hybrid)

       do ie=nets,nete
          tmp1(ie) = MAXVAL(elem(ie)%state%Q(:,:,:,q,nm1))
       enddo
       qvmax_p(q) = ParallelMax(tmp1,hybrid)

       do ie=nets,nete
          tmp1(ie) = SUM(elem(ie)%state%Q(:,:,:,q,nm1))
       enddo
       qvsum_p(q) = ParallelSum(tmp1,hybrid)
    ...
    do q=1,qsize
       ...
       enddo
       Qmass_added(q) = parallelsum(tmp1,hybrid)*scale   
    enddo
...
!   Compute Energies at time1 and time2 (half levels between leapfrog steps)
    ...
    if(hybrid%par%masterproc .and. hybrid%ithr==0) then 
       ...
       if (integration == "explicit") then
          ...
          ! forcing is applied after complete dynamics timestep.  
          ! We only compute the observed changes
          ! KE(t+.5*)-KE(t+5)     t+.5 = pre-forcing and robert
          !                       t+.5*= after forcing and robert  
          ! TIME-SPLIT TERMS
          ! for leapfrog, with staggered-in-time formulas, correct to use dt
          ! since influence of adjustments to t+1 are felt at 1/2 strength. 
          ! with non-staggered formulas, need to use 2*dt
          ! what about in the qsplit>1 case?  just one of the problems with
          ! subcycled leapfrog.   
          dt_split = dt
          if (.not.subcycle) then
             if (energy_fixer == -1 .or. energy_fixer == 3 .or. energy_fixer == 4 ) then
                dt_split = 2*dt
             endif
          endif
!!$          if (ftype==1) then
             ! time split forcing - applied in physics, not seen by dynamics
             write(iulog,'(a)')  '                     Robert+fixer '
             write(iulog,'(a,2e23.15,a)') 'dKE/dt(W/m^2): ',(KEner(4)-KEner(3))/dt_split
             write(iulog,'(a,2e23.15,a)') 'dIE/dt(W/m^2): ',(IEner(4)-IEner(3))/dt_split
             write(iulog,'(a,2e23.15,a)') 'dPE/dt(W/m^2): ',(PEner(4)-PEner(3))/dt_split
!!$          else
!!$             write(iulog,'(a)')  '**FORCING**            Physics              Robert+fixer '
!!$             do q=1,qsize   
!!$                write(iulog,'(a,i1,a,2e23.15)') '       dQ',q,'/dt: ',(Qmass(q,3)-Qmass(q,2))/dt,(Qmass(q,4)-Qmass(q,3))/dt
!!$             enddo
!!$             write(iulog,'(a,2e23.15,a)') 'dKE/dt(W/m^2): ',(KEner(3)-KEner(2))/dt_split,(KEner(4)-KEner(3))/dt_split
!!$             write(iulog,'(a,2e23.15,a)') 'dIE/dt(W/m^2): ',(IEner(3)-IEner(2))/dt_split,(IEner(4)-IEner(3))/dt_split
!!$             write(iulog,'(a,2e23.15,a)') 'dPE/dt(W/m^2): ',(PEner(3)-PEner(2))/dt_split,(PEner(4)-PEner(3))/dt_split
!!$          endif
       endif
       if (TOTE0>0) then
...
subroutine prim_energy_halftimes(elem,hvcoord,tl,n,nets,nete,tQ)
! 
...
!
    ...
    real (kind=real_kind), dimension(nv,nv,nlev)  :: dpt1,dpt2   ! delta pressure
    ...
    do ie=nets,nete
       ...
       do k=1,nlev
          ...
          do j=1,nv
             ...
             if (tstagger) then
                suml(i,j) = suml(i,j) + cp_star1*elem(ie)%state%T(i,j,k,t2) *dpt1(i,j,k)/2 
                suml(i,j) = suml(i,j) + Cp_star2*elem(ie)%state%T(i,j,k,t1) *dpt2(i,j,k)/2
                suml2(i,j) = suml2(i,j) + (cp_star1-cp)*elem(ie)%state%T(i,j,k,t2) *dpt1(i,j,k)/2 
                suml2(i,j) = suml2(i,j) + (cp_star2-cp)*elem(ie)%state%T(i,j,k,t1) *dpt2(i,j,k)/2
             else
                suml(i,j) = suml(i,j) + Cp_star2*elem(ie)%state%T(i,j,k,t2) *dpt2(i,j,k)
                suml2(i,j) = suml2(i,j) + (cp_star2-cp)*elem(ie)%state%T(i,j,k,t2) *dpt2(i,j,k)
             endif
          enddo
          enddo
       enddo
...
subroutine prim_diag_scalars(elem,hvcoord,tl,n,nets,nete)
! 
!  called at the end of a timestep, before timelevel update.  Solution known at
...
!
    ...
    integer :: ie,k,q,nm_f
    real (kind=real_kind), dimension(nv,nv)  :: ps         ! pressure
    real (kind=real_kind), dimension(nv,nv)  :: dp         ! delta pressure
    real (kind=real_kind), dimension(nv,nv)  :: E
    ...
    !
    !  RK2 foward scheme.  compute everything at t2
    !
    if (tracer_advection_formulation==TRACERADV_TOTAL_DIVERGENCE .and. &
         tstep_type==1) then
