module filter_mod
  ...
  type, public :: filter_t
     sequence
     real (kind=real_kind) :: FmatV(nv,nv)    ! Filter matrix
...
contains
  ...
  ! function T...
  ! ==================================
  function taylor_filter_create(Tp, Tv,mu, gquadp,gquadv) result(flt)
    use kinds, only : real_kind
    use quadrature_mod, only : quadrature_t, legendre, quad_norm
    real (kind=real_kind), dimension(:) :: Tv,Tp          ! transfer fn       
    real (kind=real_kind), intent(in)   :: mu             ! filter viscosity
    type (quadrature_t)  , intent(in)   :: gquadp,gquadv  ! quadrature points and wts

    type (filter_t)                   :: flt

    ! Local variables 

    real (kind=real_kind), dimension(:), allocatable   :: gammaP,gammaV  ! normalization factor
    ...
    real (kind=real_kind), dimension(:,:), allocatable :: AinvP,AinvV   ! physical-spectral
    ...
    allocate(AinvV(nv,nv))

    gammaP=quad_norm(gquadP,np)
    gammaV=quad_norm(gquadV,nv)
    ...
    do j=1,nv
       if(Debug) print *,'taylor_filter_create: gammaV(j) := ',gammaV(j)
       legV(:,j)=legendre(gquadV%points(j),nv-1)
    end do
    ...
    do n=1,nv
       do j=1,nv
          AinvV(j,n)=legV(n,j)*gquadV%weights(j)/gammaV(n)
       end do
    end do

    call filter_matrix_create(AP,Tp,AinvP,flt%FmatP,np,mu)
    call filter_matrix_create(AV,Tv,AinvV,flt%FmatV,nv,mu)
  ...
  function fm_filter_create(Tp,Tv, mu, gquadP,gquadV) result(flt)
    use quadrature_mod, only : quadrature_t, legendre

    real (kind=real_kind),   dimension(:) :: Tp,Tv    ! transfer fn
    real (kind=real_kind), intent(in) :: mu       ! filter viscosity
    type (quadrature_t)  , intent(in) :: gquadP,gquadV    ! quadrature points/wts
    ...
    do j=1,nv
       ...
       AV(2,j)=legV(2,j)
       do n=3,nv
          AV(n,j)=legV(n,j)-legV(n-2,j)
       end do
    end do

    call filter_matrix_create(AP,Tp,flt%FmatP,np,mu)
    call filter_matrix_create(AV,Tv,flt%FmatV,nv,mu)
...
11     format(e22.15,3x,e22.15)
  ...
  subroutine filter_V(v,flt) 
    type (filter_t), intent(in)          :: flt
    real(kind=real_kind), intent(inout)  :: v(nv,nv)

    ! Local

    integer  :: i,j,l
    logical, parameter :: UseUnroll = .FALSE.
    real(kind=real_kind) :: temptv(nv,nv)
    real(kind=real_kind)  sumx00,sumx01
    real(kind=real_kind)  sumx10,sumx11
    !JMD    integer npts
    ...
    if(MODULO(nv,2) == 0 .and. UseUnroll ) then 
       do j=1,nv,2
          do l=1,nv,2
             ...
             do i=1,nv
                sumx00 = sumx00 + flt%FmatV(i,l  )*v(i,j  )
                sumx01 = sumx01 + flt%FmatV(i,l+1)*v(i,j  )
                sumx10 = sumx10 + flt%FmatV(i,l  )*v(i,j+1)
                sumx11 = sumx11 + flt%FmatV(i,l+1)*v(i,j+1)
             end do

             temptV(j  ,l  ) = sumx00
             temptV(j  ,l+1) = sumx01
             temptV(j+1,l  ) = sumx10
             temptV(j+1,l+1) = sumx11
       ...
       do j=1,nv,2
          do i=1,nv,2
             ...
             do l=1,nv
                sumx00 = sumx00 +  flt%FmatV(l,j  )*temptV(l,i  )
                sumx01 = sumx01 +  flt%FmatV(l,j+1)*temptV(l,i  )
                sumx10 = sumx10 +  flt%FmatV(l,j  )*temptV(l,i+1)
                sumx11 = sumx11 +  flt%FmatV(l,j+1)*temptV(l,i+1)
             end do
  ...
  subroutine preq_filter(elem, edge3p1, flt, hybrid, nfilt, nets, nete)
    ...
    if (flt%type == "bv") then
       do ie=nets,nete          
          ...
          do j=1,nv
             do i=1,nv
                elem(ie)%state%ps_v(i,j,nfilt) = elem(ie)%mv(i,j)*elem(ie)%state%ps_v(i,j,nfilt)
             end do
          end do
          do k=1,nlev
             do j=1,nv
                do i=1,nv
                   elem(ie)%state%T(i,j,k,nfilt)   = elem(ie)%mv(i,j)*elem(ie)%state%T(i,j,k,nfilt)
                   elem(ie)%state%v(i,j,1,k,nfilt) = elem(ie)%mv(i,j)*elem(ie)%state%v(i,j,1,k,nfilt)
                   elem(ie)%state%v(i,j,2,k,nfilt) = elem(ie)%mv(i,j)*elem(ie)%state%v(i,j,2,k,nfilt)
                end do
             end do
          end do
       ...
       do ie=nets,nete
          ...
          do j=1,nv
             do i=1,nv
                elem(ie)%state%ps_v(i,j,nfilt) = elem(ie)%rmv(i,j)*elem(ie)%state%ps_v(i,j,nfilt)
             end do
          end do
          ...
          do k=1,nlev
             do j=1,nv
                do i=1,nv
                   elem(ie)%state%T(i,j,k,nfilt)   = elem(ie)%rmv(i,j)*elem(ie)%state%T(i,j,k,nfilt)
                   elem(ie)%state%v(i,j,1,k,nfilt) = elem(ie)%rmv(i,j)*elem(ie)%state%v(i,j,1,k,nfilt)
                   elem(ie)%state%v(i,j,2,k,nfilt) = elem(ie)%rmv(i,j)*elem(ie)%state%v(i,j,2,k,nfilt)
                end do
             end do
          end do
...
!$OMP BARRIER
  ...
  subroutine prim_filt_bv(ps,T,v,flt)
    ...
    real(kind=real_kind), intent(inout) :: ps(nv,nv)
    real(kind=real_kind), intent(inout) :: T(nv,nv,nlev)
    real(kind=real_kind), intent(inout) :: v(nv,nv,2,nlev)

    type (filter_t), intent(in) :: flt

    integer :: k

    call filter_V(ps(:,:),flt)
    do k=1,nlev
       call filter_V(T(:,:,k),flt)
       call filter_V(v(:,:,1,k),flt)
       call filter_V(v(:,:,2,k),flt)
    end do
  ...
  subroutine prim_filt_fm(ps,T,div,v,flt,D,Dinv)
    ...
    real(kind=real_kind), intent(inout) :: ps(nv,nv)
    real(kind=real_kind), intent(inout) :: T(nv,nv,nlev)
    real(kind=real_kind), intent(inout) :: div(nv,nv,nlev)
    real(kind=real_kind), intent(inout) :: v(nv,nv,2,nlev)

    type (filter_t), intent(in)      :: flt
    real(kind=real_kind), intent(in) :: D(2,2,nv,nv)
    real(kind=real_kind), intent(in) :: Dinv(2,2,nv,nv)

    integer :: i,j,k
    real(kind=real_kind) :: v1,v2
    real(kind=real_kind) :: usph(nv,nv)
    real(kind=real_kind) :: vsph(nv,nv)

    call filter_V(ps(:,:),flt)
    do k=1,nlev
       call filter_V(T(:,:,k),flt)
       !     call filter_V(div(:,:,k),flt)
       ...
       call filter_V(usph(:,:),flt)
       call filter_V(vsph(:,:),flt)
  ...
  subroutine prim_filter(elem, edge_3lp1,flt,nfilt,nets,nete,hybrid)
    ...
    real (kind=real_kind) :: u(nv,nv)
    real (kind=real_kind) :: v(nv,nv)
    real (kind=real_kind) :: v1,v2
    real (kind=real_kind) :: tmp(nv,nv)

    integer i,j,k,ie
    integer kptr

    real (kind=real_kind), dimension(:,:), pointer :: mv,rmv
    ...
    if (flt%type == "bv") then
...
#endif
          call filter_v(elem(ie)%state%lnps(:,:,nfilt),flt)

          elem(ie)%state%lnps(:,:,nfilt) = mv*elem(ie)%state%lnps(:,:,nfilt)

          do k=1,nlev
             call filter_V(elem(ie)%state%T(:,:,k,nfilt),flt)
             call filter_V(elem(ie)%state%v(:,:,1,k,nfilt),flt)
             call filter_V(elem(ie)%state%v(:,:,2,k,nfilt),flt)
             do j=1,nv
                do i=1,nv
                   elem(ie)%state%T(i,j,k,nfilt)   = mv(i,j)*elem(ie)%state%T(i,j,k,nfilt)
                   elem(ie)%state%v(i,j,1,k,nfilt) = mv(i,j)*elem(ie)%state%v(i,j,1,k,nfilt)
                   elem(ie)%state%v(i,j,2,k,nfilt) = mv(i,j)*elem(ie)%state%v(i,j,2,k,nfilt)
                end do
             end do
          end do
       ...
       do ie=nets,nete
          ...
          elem(ie)%state%lnps(:,:,nfilt) = rmv*elem(ie)%state%lnps(:,:,nfilt)
          ...
          do k=1,nlev
             do j=1,nv
                do i=1,nv
                   elem(ie)%state%T(i,j,k,nfilt)   = rmv(i,j)*elem(ie)%state%T(i,j,k,nfilt)
                   elem(ie)%state%v(i,j,1,k,nfilt) = rmv(i,j)*elem(ie)%state%v(i,j,1,k,nfilt)
                   elem(ie)%state%v(i,j,2,k,nfilt) = rmv(i,j)*elem(ie)%state%v(i,j,2,k,nfilt)
                end do
             end do
          end do
