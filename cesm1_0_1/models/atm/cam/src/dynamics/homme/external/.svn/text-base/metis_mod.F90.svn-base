...
contains 
  ...
  subroutine genmetispart(GridEdge, GridVertex)
    ...
    if(OutputFiles) then
       do k=1,nelem
          write(10,*) GridVertex(k)%partition
       end do
       close(10)
    end if
#endif
...
100 format("genmetispart:  Partitioning ",i4," elements, using method: ",i2," into ",i4," pieces")
  ...
  subroutine CreateMetisGraph(GridVertex,xadj,adjncy,adjwgt)
    ...
    max_neigh = MAXVAL(GridVertex(:)%degree)
    ...
    do i=1,nelem
       ...
       jj=1
       neigh_list=0
       do j=1,GridVertex(i)%degree
          if(GridVertex(i)%wgtV(j) .gt. 0) then 
             adjncy(ii+jj-1)   = GridVertex(i)%nbrs(j)
	     neigh_list(jj)    = GridVertex(i)%nbrs(j)
             adjwgt(ii+jj-1)   = GridVertex(i)%wgtV(j)*EdgeWeight
             neigh_wgt(jj)     = GridVertex(i)%wgtV(j)*EdgeWeight
             jj=jj+1
          endif
       enddo
       call sort(max_neigh,neigh_list,index)
       degree       = COUNT(GridVertex(i)%wgtV(:) .gt. 0) 
