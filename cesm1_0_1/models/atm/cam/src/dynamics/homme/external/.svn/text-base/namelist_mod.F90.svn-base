...
module namelist_mod
  ...
  !-----------------
  use dimensions_mod, only : ne,np,nv, nnodes, nmpi_per_node, npart, qsize, qsize_d
  !-----------------
#ifdef CAM
  use time_mod, only : homme_nsplit, smooth
#else
  use time_mod, only : tstep, ndays,nmax, nendstep,secpday, smooth, secphr
#endif
  !-----------------
  ...
  use common_io_mod, only : &
       ...
       varname_len,         &
       infilenames,         &
       MAX_INFILES, MAX_VECVARS, &
       vector_uvars, vector_vvars
#ifdef _PRIM
...
! ============================================
  ...
  subroutine readnl(par)
...
#ifdef CAM                      
    namelist  /ctl_nl/ HOMME_NSPLIT               ! number of dynamics steps per physics timestep
#else
    ...
    namelist /aquaplanet_nl/cool_ampl, &
                        ...
                        noisef
#endif
    integer :: nlat, nlon, gridtype, interp_type
...
#ifdef CAM
    character(len=MAX_STRING_LEN) :: homme_topology
    integer :: homme_partmethod
    integer :: homme_ne
    integer :: unitn
    ...
    if (par%masterproc) then
      ...
      PARTMETHOD = SFCURVE
...
#else
          write(errstr,*) 'readnl: runtype= ',runtype,' invalid in this mode'
          call abortmp(errstr)
#endif
       ...
       if ((integration == "semi_imp").or.(integration == "full_imp")) then
          ...
          print *,'HPYERIVS order = ',hypervis_order
...
#endif
#ifdef PIO_INTERP
!      Default interpolation grid  (0 = auto compute based on ne,nv)
       nlat =  0
       nlon = 0
       gridtype = 2	
       interp_type = 0
       ...
       vector_uvars(1:2) = (/'U      ','UTGWORO'/)
       vector_vvars(1:2) = (/'V      ','VTGWORO'/)
#endif
...
!=======================================================================================================!
...
#ifdef CAM
    if(homme_partmethod /= -1) partmethod = homme_partmethod
    if(homme_ne /= -1) ne = homme_ne
    if(homme_topology .ne. 'none') topology = homme_topology
#endif
...
#ifdef CAM
    call MPI_bcast( HOMME_NSPLIT,1,MPIinteger_t,par%root,par%comm,ierr)
#else
    ...
    nEndStep = nmax
#endif
    call MPI_bcast(ftype     ,1,MPIinteger_t   ,par%root,par%comm,ierr) 
...
#endif
#ifdef PIO_INTERP
    call MPI_bcast(nlat , 1,MPIinteger_t,par%root,par%comm,ierr)
    call MPI_bcast(nlon , 1,MPIinteger_t,par%root,par%comm,ierr)
    call MPI_bcast(gridtype , 1,MPIinteger_t,par%root,par%comm,ierr)
    ...
    call set_interp_parameter('gridtype',gridtype)
    ...
    if (nlon==0 .or. nlat==0) then
       ...
       call set_interp_parameter('auto',4*ne*(nv-1))
       ...
       call set_interp_parameter('nlat',nlat)
       call set_interp_parameter('nlon',nlon)
    endif
    ...
    ! subcycling only supported with RK-SSP tracer integration    
    if (qsplit>1) then
       if (tstep_type /= 1) then
          call abortmp('tracer/dynamics subcycling requires tstep_type = 1')
       endif
    endif
    if (tstep_type == 1 ) then
       if (tracer_advection_formulation/=TRACERADV_TOTAL_DIVERGENCE) then
          call abortmp('RK-SSP tracer integration requires total div. formulation')
       endif
    endif
    ...
    if (qsplit>1) then
       if (energy_fixer /= -1 .and. energy_fixer /= 3 .and. energy_fixer /= 4) then
          call abortmp("ERROR: subcycling requires non-staggered-in-time energy fixer")
       endif
    endif
...
!=======================================================================================================!
    ...
    if(nu_s<0) nu_s=nu	
    if(nu_q<0) nu_q=nu	
    ! tracer viscosity subcycling must be >= dynamics viscosity subcycling
    hypervis_subcycle_q= max(hypervis_subcycle_q,hypervis_subcycle)
    ...
    if (par%masterproc) then
       ...
       write(iulog,*)"readnl: ne,np,nv      = ",NE,np,nv
...
#ifdef CAM
       write(iulog,*)"readnl: nsplit         = ", HOMME_NSPLIT
#else
       ...
       write(iulog,*)"limiter_option        = ",limiter_option
       ...
       write(iulog,*)"viscosity:  nu        = ",nu
       write(iulog,*)"viscosity:  nu_s      = ",nu_s
       write(iulog,*)"viscosity:  nu_q      = ",nu_q
       write(iulog,*)"viscosity:  nu_p      = ",nu_p
       write(iulog,*)"viscosity:  nu_top      = ",nu_top
...
#ifdef PIO_INTERP
       write(iulog,*)" analysis interp nlat = ",nlat
       write(iulog,*)" analysis interp nlon = ",nlon
       write(iulog,*)" analysis interp gridtype = ",gridtype
