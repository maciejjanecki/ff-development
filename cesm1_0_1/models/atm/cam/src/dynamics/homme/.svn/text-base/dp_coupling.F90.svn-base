...
!===============================================================================
  subroutine d_p_coupling(phys_state, phys_tend, pbuf, dyn_out)
...
! !OUTPUT PARAMETERS:
    ...
    type(pbuf_fld),      intent(inout), dimension(:)     :: pbuf
...
! LOCAL VARIABLES
    ...
    real (kind=real_kind)    :: q_tmp(npsq,pver,pcnst,nelemd) ! temporary to hold advected constiuants
    ...
    tl_f = TimeLevel%nm1  ! process split physics
    if (qsplit>1) tl_f = TimeLevel%n0  ! time split physics
    ...
    do ie=1,nelemd
       ...
       call UniquePoints(elem(ie)%idxP, nlev,pcnst, elem(ie)%state%Q(:,:,:,:,tl_f), Q_tmp(1:ncols,:,:,ie))
    end do
...
!$omp parallel do private (lchnk, ncols, pgcols, icol, idmb1, idmb2, idmb3, ie, ioff, ilyr, m)
       do lchnk=begchunk,endchunk
          ...
          do icol=1,ncols
             ...
             phys_state(lchnk)%ps(icol)=ps_tmp(ioff,ie)*100.D0
...
!$omp parallel do private (ie, bpter, icol, ilyr, m)
       do ie=1,nelemd
          ...
          do icol=1,elem(ie)%idxP%NumUniquePts
             ...
             bbuffer(bpter(icol,0))   = ps_tmp(icol,ie)*100._r8
             ...
             do ilyr=1,pver
                ...
                do m=1,pcnst
                   bbuffer(bpter(icol,ilyr)+3+m) = Q_tmp(icol,ilyr,m,ie)
                end do
             end do
...
!$omp parallel do private (lchnk, ncols, cpter, icol, ilyr, m)
       do lchnk = begchunk,endchunk
          ...
          do icol=1,ncols
             ...
             do ilyr=1,pver
                ...
                do m=1,pcnst
                   phys_state(lchnk)%q  (icol,ilyr,m) = cbuffer(cpter(icol,ilyr)+3+m)
                end do
    ...
    call derived_phys(phys_state,phys_tend,pbuf)
...
!$omp parallel do private (lchnk, ncols, ilyr, icol)
   ...
   if (write_inithist() ) then
    do lchnk=begchunk,endchunk
        ...
        call outfld('PHIS&IC',phys_state(lchnk)%phis,pcols,lchnk)
        do m=1,pcnst
  ...
  subroutine p_d_coupling(phys_state, phys_tend, pbuf, dyn_in)
...
!$omp parallel do private (ie, bpter, icol, ilyr, m)
  ...
  subroutine derived_phys(phys_state, phys_tend, pbuf)
...
#if 0
! This wont match HOMME exactly.  HOMME computes KE at half levels
! HOMME SE includes cp_star (physics SE uses cp )
