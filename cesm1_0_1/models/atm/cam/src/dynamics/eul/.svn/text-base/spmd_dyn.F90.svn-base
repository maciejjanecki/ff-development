...
!========================================================================
  ...
  subroutine spmd_dyn_defaultopts(npr_yz_out, geopktrans_out,    &
               geopkblocks_out,                                  &
               dyn_conservative_out, force_2d_out,               &
               modcomm_transpose_out, modcomm_geopk_out,         &
               modcomm_gatscat_out, dyn_alltoall_out,            &
               dyn_allgather_out, dyn_equi_by_col_out,           &
...
! FV-only arguments
     ...
     logical, intent(out), optional :: dyn_conservative_out
...
!-----------------------------------------------------------------------
     ...
     if ( present(dyn_npes_out) ) then
       dyn_npes_out =  def_npes
     endif
     if ( present(dyn_npes_stride_out) ) then
       dyn_npes_stride_out =  def_npes_stride
     endif
...
!========================================================================
  ...
  subroutine spmd_dyn_setopts(npr_yz_in, geopktrans_in,       &
               geopkblocks_in,                                &
               dyn_conservative_in, force_2d_in,              &
               modcomm_transpose_in, modcomm_geopk_in,        &
               modcomm_gatscat_in, dyn_alltoall_in,           &
               dyn_allgather_in, dyn_equi_by_col_in,          &
...
! FV-only arguments
     ...
     logical, intent(in), optional :: dyn_conservative_in
     ...
     integer, intent(in), optional :: modcomm_gatscat_in
...
!
     if ( present(dyn_npes_in) ) then
       dyn_npes = dyn_npes_in
       if ((dyn_npes.lt.min_npes).or. &
           (dyn_npes.gt.max_npes)) then
         write(iulog,*)                                          &
           'SPMD_DYN_SETOPTS:  ERROR:  dyn_npes=', &
           dyn_npes_in,                              &
           '  is out of range.  It must be between ',        &
...
!
     if ( present(dyn_npes_stride_in) ) then
       dyn_npes_stride = dyn_npes_stride_in
       if ((dyn_npes_stride.lt.min_npes_stride).or. &
           (dyn_npes_stride.gt.max_npes_stride)) then
         write(iulog,*)                                          &
           'SPMD_DYN_SETOPTS:  ERROR:  dyn_npes_stride=', &
           dyn_npes_stride_in,                              &
           '  is out of range.  It must be between ',        &
