...
!
! $Id: cprnc.F90 4236 2007-05-02 18:15:57Z erik $
!-------------------------------------------------------------------------------------------
   use utils,         only: endrun, lenchr
   use specialvalues, only: uninit_int
   use netcdfids,     only: unlimdimid, dateid, datesecid, nstephid, date_writtenid, time_writtenid, timevid, timedid, timestr
...
!
   ...
   cvsid = '$Id: cprnc.F90 4236 2007-05-02 18:15:57Z erik $'
...
! Set debug in nfwrappers
   if (verbose) call wrap_setdebug(2)
!
...
!
      ...
      call wrap_inq_dimid  (nfid(n), timestr, timedid(n))
      call wrap_inq_dimlen (nfid(n), timedid(n), ntime(n))
      ...
      call wrap_inq_varid (nfid(n), timestr, timevid(n), -1)
!
! Ensure variable named "time" is what we think it is
!
      if (timevid(n) == -1) then
         write(6,*) 'time variable not on ',trim(fname(n))
      else
         call wrap_inq_var (nfid(n), timevid(n), name, xtype, ndims(n), dimids(:,n), natts)
...
!
   ...
   write(6,'(a)') 'cprnc completed successfully'
...
!
   do ka = 1,2
      kb = mod(ka,2) + 1
      call wrap_inq_ndims (nfid(ka), ndims)
   do n=1,ndims
      dimlen1 = -1
      dimlen2 = -1
      call wrap_inq_dimname (nfid(ka), n, dimname)
      call wrap_inq_dimlen (nfid(ka), n, dimlen1)
      call wrap_inq_dimid (nfid(kb), dimname, dimid)
      call wrap_inq_dimlen (nfid(kb), dimid, dimlen2)
      if (dimlen1 == dimlen2) then
         if (ka == 1) &
         write(6,101) ' Dimensions size same ...',dimlen1,' : ',dimname(1:lenchr(dimname))
      elseif (dimlen2 == -1) then
         if (ka == 1) &
         write(6,101) ' Dimension file1 only ---',dimlen1,' : ',dimname(1:lenchr(dimname))
         if (ka == 2) &
         write(6,101) ' Dimension file2 only ---',dimlen1,' : ',dimname(1:lenchr(dimname))
      else
         if (ka == 1) &
         write(6,102) ' Dimensions different ***',dimlen1,dimlen2,' : ',dimname(1:lenchr(dimname))
      end if
   end do
   end do
