...
subroutine atm_register_esmf(comp, rc)
   ...
   call ESMF_GridCompSetEntryPoint(comp, ESMF_SETINIT, atm_init_esmf, ESMF_SINGLEPHASE, rc)
   if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
   call ESMF_GridCompSetEntryPoint(comp, ESMF_SETRUN, atm_run_esmf, ESMF_SINGLEPHASE, rc)
   if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
   call ESMF_GridCompSetEntryPoint(comp, ESMF_SETFINAL, atm_final_esmf, ESMF_SINGLEPHASE, rc)
   if(rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, terminationflag=ESMF_ABORT)
...
subroutine atm_init_esmf(comp, import_state, export_state, EClock, rc)
   ...
   if (phase == 1) then
      call seq_cdata_init(cdata,ATMID,ggrid,gsmap,infodata,'datm')
   else
