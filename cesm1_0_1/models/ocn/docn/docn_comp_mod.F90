...
! !USES:
  ...
  use seq_timemgr_mod
  use seq_flds_indices , only: nflds_o2x, &
                               nflds_x2o
...
!-------------------------------------------------------------------------------
    ...
    if (my_task == master_task.and.exists) then
       logUnit = shr_file_getUnit()
       call shr_file_setIO('ocn_modelio.nml',logUnit)
    else
       logUnit = 6
    endif
...
!   enddo
   ...
   if (write_restart) then
      ...
      write(rest_file,"(2a,i4.4,a,i2.2,a,i2.2,a,i5.5,a)") &
        trim(case_name), '.docn.r.', yy,'-',mm,'-',dd,'-',currentTOD,'.nc'
      write(rest_file_strm,"(2a,i4.4,a,i2.2,a,i2.2,a,i5.5,a)") &
        trim(case_name), '.docn.rs1.', yy,'-',mm,'-',dd,'-',currentTOD,'.bin'
      if (my_task == master_task) then
         nu = shr_file_getUnit()
         open(nu,file=trim(rpfile),form='formatted')
