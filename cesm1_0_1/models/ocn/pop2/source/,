...
!maltrud variable restoring
   ...
   type(forcing_monthly_every_ts) :: &
      ...
      pox_flux_monthly,          & ! surface PO4x species flux, added to phosphate pool
      siy_flux_monthly             ! surface PO4x species flux, added to phosphate pool
   ...
   integer (int_kind), parameter :: &
      ndep_shr_stream_var_cnt = 4, & ! number of variables in ndep shr_stream
      ndep_shr_stream_no_ind  = 1, & ! index for NO forcing
      ndep_shr_stream_nh_ind  = 2, & ! index for NH forcing
      ndep_shr_stream_po_ind  = 3, & ! index for PO forcing
      ndep_shr_stream_si_ind  = 4    ! index for SI forcing
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_POx_FLUX,     &! tavg id for pox flux
      tavg_SIy_FLUX       ! tavg id for siy flux
...
!-----------------------------------------------------------------------
   ...
   type(tracer_read) :: &
      ...
      pox_flux_monthly_input,    & ! namelist input for pox_flux_monthly
      siy_flux_monthly_input       ! namelist input for siy_flux_monthly
...
!-----------------------------------------------------------------------
   ...
   namelist /ecosys_nml/ &
      ...
      pox_flux_monthly_input, siy_flux_monthly_input, &
...
!-----------------------------------------------------------------------
   ...
   call init_forcing_monthly_every_ts(pox_flux_monthly)
   call init_forcing_monthly_every_ts(siy_flux_monthly)
...
!maltrud variable restoring
   ...
   pox_flux_monthly_input%filename     = 'unknown'
   pox_flux_monthly_input%file_varname = 'pox_flux'
   pox_flux_monthly_input%scale_factor = c1
   pox_flux_monthly_input%default_val  = c0
   pox_flux_monthly_input%file_fmt     = 'bin'

   siy_flux_monthly_input%filename     = 'unknown'
   siy_flux_monthly_input%file_varname = 'siy_flux'
   siy_flux_monthly_input%scale_factor = c1
   siy_flux_monthly_input%default_val  = c0
   siy_flux_monthly_input%file_fmt     = 'bin'
...
!maltrud variable restoring
   ...
   call broadcast_scalar(pox_flux_monthly_input%filename, master_task)
   call broadcast_scalar(pox_flux_monthly_input%file_varname, master_task)
   call broadcast_scalar(pox_flux_monthly_input%scale_factor, master_task)
   call broadcast_scalar(pox_flux_monthly_input%default_val, master_task)
   call broadcast_scalar(pox_flux_monthly_input%file_fmt, master_task)

   pox_flux_monthly%input = pox_flux_monthly_input

   call broadcast_scalar(siy_flux_monthly_input%filename, master_task)
   call broadcast_scalar(siy_flux_monthly_input%file_varname, master_task)
   call broadcast_scalar(siy_flux_monthly_input%scale_factor, master_task)
   call broadcast_scalar(siy_flux_monthly_input%default_val, master_task)
   call broadcast_scalar(siy_flux_monthly_input%file_fmt, master_task)

   siy_flux_monthly%input = siy_flux_monthly_input
...
!-----------------------------------------------------------------------
   ...
   call define_tavg_field(tavg_POx_FLUX,'POx_FLUX',2,                  &
                          long_name='Flux of PO4x from Atmosphere',    &
                          units='nmol/cm^2/s', grid_loc='2110',        &
                          coordinates='TLONG TLAT time')

   call define_tavg_field(tavg_SIy_FLUX,'SIy_FLUX',2,                  &
                          long_name='Flux of SIy from Atmosphere',    &
                          units='nmol/cm^2/s', grid_loc='2110',        &
                          coordinates='TLONG TLAT time')
...
!mj   thetaC_sp   = 0.24_r8
!mj   thetaC_diat = 0.24_r8
!mj   thetaC_diaz = 0.24_r8
...
!   Tfunc_sp   = exp(0.12_r8*TEMP)                                    !mj
!   Tfunc_diat = exp(0.063_r8*TEMP)                                   !mj
!   Tfunc_diaz = (exp(0.063_r8*TEMP))/(c1 + exp(28._r8 - 2._r8*TEMP)) !mj
!   Tfunc_diaz = Q_10**(((TEMP + 2.0 + T0_Kelvin)-(Tref + T0_Kelvin))/c10)
    Tfunc_diaz = Q_10**(((TEMP + T0_Kelvin)-(Tref + T0_Kelvin))/c10)
...
!mj VFeC_sp = Fe_loc / (Fe_loc + parm_sp_kFe)
    VFeC_sp = c1
...
!mj    VPO4_sp = c1 !znowu zalezy od PO4 / 8.lip.2013 bo spC wybuchalo
...
!-----------------------------------------------------------------------
 ...
 !mj    f_nut = c1
 !mj    f_nut = min(c1,(VNtot_sp*VPO4_sp)**0.5_r8)
!-----------------------------------------------------------------------
!  get small phyto photosynth. rate, phyto C biomass change, photoadapt
!-----------------------------------------------------------------------

       PCmax = PCref_sp * f_nut * Tfunc
!mj    PCmax = PCref * f_nut * Tfunc
!mj    PCmax = PCref_sp * f_nut * Tfunc_sp
...
!mj VFeC_diat = Fe_loc / (Fe_loc + parm_diat_kFe)
    VFeC_diat = c1
...
!mj Vfec_diaz = Fe_loc/(Fe_loc + diaz_kFe)
    Vfec_diaz = c1
...
!mj where (TEMP > 12.0_r8)
!mj        PCmax = PCrefDiaz * f_nut * Tfunc
!mj        where ((TEMP) < diaz_temp_thres) PCmax = c0
!mj elsewhere
!mj        PCmax = PCrefDiaz * f_nut * Tfunc_diaz
!mj        where ((TEMP) < diaz_temp_thres) PCmax = c0
!mj end where

!mj  PCmax = f_nut * Tfunc_diaz
!mj  where (TEMP < diaz_temp_thres) PCmax = c0
...
!mj   DTRACER_MODULE(:,:,nh4_ind) = -NH4_V_diat - NH4_V_sp - NITRIF + &
!mj        Q * (zoo_loss_dic + sp_loss_dic + graze_sp_dic + diat_loss_dic + &
!mj        graze_diat_dic + POC%remin(:,:,bid) + diaz_loss_dic + &
!mj        graze_diaz_dic) + DON_remin + diaz_Nexcrete - photoNH4_diaz

   DTRACER_MODULE(:,:,nh4_ind) = -NH4_V_diat - NH4_V_sp - NITRIF + &
        Q * (zoo_loss_dic + sp_loss_dic + graze_sp_dic + diat_loss_dic + &
        graze_diat_dic + diaz_loss_dic + &
...
!mj   DTRACER_MODULE(:,:,po4_ind) = RESTORE + (Qp * (zoo_loss_dic + &
!mj     sp_loss_dic + graze_sp_dic + diat_loss_dic + &
!mj     graze_diat_dic - photoC_diat)) &
!mj      + DOP_remin + diaz_loss_dip - po4_V_sp - po4_V_diaz
...
!-----------------------------------------------------------------------
!  load nox & noy & pox flux fields (if required)
!-----------------------------------------------------------------------
...
if (trim(ndep_data_type) == 'monthly-calendar' .and. &
       trim(pox_flux_monthly%input%filename) /= 'none' .and. &
       trim(pox_flux_monthly%input%filename) /= 'unknown') then
                          
      luse_INTERP_WORK = .true.

      allocate(pox_flux_monthly%DATA(nx_block,ny_block,max_blocks_clinic,1,12))
      if (trim(pox_flux_monthly%input%filename) == 'unknown') &
         pox_flux_monthly%input%filename = gas_flux_forcing_file
   
      call read_field(pox_flux_monthly%input%file_fmt, &
                      pox_flux_monthly%input%filename, &
                      pox_flux_monthly%input%file_varname, &
                      WORK_READ)
   
      !$OMP PARALLEL DO PRIVATE(iblock, n)
      do iblock=1,nblocks_clinic
      do n=1,12
         pox_flux_monthly%DATA(:,:,iblock,1,n) = WORK_READ(:,:,n,iblock)
         where (.not. LAND_MASK(:,:,iblock)) &
            pox_flux_monthly%DATA(:,:,iblock,1,n) = c0
         pox_flux_monthly%DATA(:,:,iblock,1,n) = &
            pox_flux_monthly%DATA(:,:,iblock,1,n) * pox_flux_monthly%input%scale_factor
      end do
      end do
      !$OMP END PARALLEL DO

      call find_forcing_times(pox_flux_monthly%data_time, &
                              pox_flux_monthly%data_inc, pox_flux_monthly%interp_type, &
                              pox_flux_monthly%data_next, pox_flux_monthly%data_time_min_loc, &
                              pox_flux_monthly%data_update, pox_flux_monthly%data_type)

      pox_flux_monthly%has_data = .true.
   else
      pox_flux_monthly%has_data = .false.
   endif

if (trim(ndep_data_type) == 'monthly-calendar' .and. &
       trim(siy_flux_monthly%input%filename) /= 'none' .and. &
       trim(siy_flux_monthly%input%filename) /= 'unknown') then

      luse_INTERP_WORK = .true.

      allocate(siy_flux_monthly%DATA(nx_block,ny_block,max_blocks_clinic,1,12))
      if (trim(siy_flux_monthly%input%filename) == 'unknown') &
         siy_flux_monthly%input%filename = gas_flux_forcing_file

      call read_field(siy_flux_monthly%input%file_fmt, &
                      siy_flux_monthly%input%filename, &
                      siy_flux_monthly%input%file_varname, &
                      WORK_READ)

      !$OMP PARALLEL DO PRIVATE(iblock, n)
      do iblock=1,nblocks_clinic
      do n=1,12
         siy_flux_monthly%DATA(:,:,iblock,1,n) = WORK_READ(:,:,n,iblock)
         where (.not. LAND_MASK(:,:,iblock)) &
            siy_flux_monthly%DATA(:,:,iblock,1,n) = c0
         siy_flux_monthly%DATA(:,:,iblock,1,n) = &
            siy_flux_monthly%DATA(:,:,iblock,1,n) * siy_flux_monthly%input%scale_factor
      end do
      end do
      !$OMP END PARALLEL DO

      call find_forcing_times(siy_flux_monthly%data_time, &
                              siy_flux_monthly%data_inc, siy_flux_monthly%interp_type, &
                              siy_flux_monthly%data_next, siy_flux_monthly%data_time_min_loc, &
                              siy_flux_monthly%data_update, siy_flux_monthly%data_type)

      siy_flux_monthly%has_data = .true.
   else
      siy_flux_monthly%has_data = .false.
   endif
...
!-----------------------------------------------------------------------
!  calculate nox and nhy and pox and siy fluxes if necessary
!-----------------------------------------------------------------------
...
if (pox_flux_monthly%has_data) then
      if (thour00 >= pox_flux_monthly%data_update) then
         tracer_data_names = pox_flux_monthly%input%file_varname
         tracer_bndy_loc   = field_loc_center
         tracer_bndy_type  = field_type_scalar
         tracer_data_label = 'POx Flux'
         call update_forcing_data(pox_flux_monthly%data_time,    &
            pox_flux_monthly%data_time_min_loc,  pox_flux_monthly%interp_type, &
            pox_flux_monthly%data_next,          pox_flux_monthly%data_update, &
            pox_flux_monthly%data_type,          pox_flux_monthly%data_inc,    &
            pox_flux_monthly%DATA(:,:,:,:,1:12), pox_flux_monthly%data_renorm, &
            tracer_data_label,                   tracer_data_names,            &
            tracer_bndy_loc,                     tracer_bndy_type,             &
            pox_flux_monthly%filename,           pox_flux_monthly%input%file_fmt)
      endif
      call interpolate_forcing(INTERP_WORK,     &
         pox_flux_monthly%DATA(:,:,:,:,1:12), &
         pox_flux_monthly%data_time,         pox_flux_monthly%interp_type, &
         pox_flux_monthly%data_time_min_loc, pox_flux_monthly%interp_freq, &
         pox_flux_monthly%interp_inc,        pox_flux_monthly%interp_next, &
         pox_flux_monthly%interp_last,       0)
      STF_MODULE(:,:,po4_ind,:) = INTERP_WORK(:,:,:,1)
   endif

if (siy_flux_monthly%has_data) then
      if (thour00 >= siy_flux_monthly%data_update) then
         tracer_data_names = siy_flux_monthly%input%file_varname
         tracer_bndy_loc   = field_loc_center
         tracer_bndy_type  = field_type_scalar
         tracer_data_label = 'SIy Flux'
         call update_forcing_data(siy_flux_monthly%data_time,    &
            siy_flux_monthly%data_time_min_loc,  siy_flux_monthly%interp_type, &
            siy_flux_monthly%data_next,          siy_flux_monthly%data_update, &
            siy_flux_monthly%data_type,          siy_flux_monthly%data_inc,    &
            siy_flux_monthly%DATA(:,:,:,:,1:12), siy_flux_monthly%data_renorm, &
            tracer_data_label,                   tracer_data_names,            &
            tracer_bndy_loc,                     tracer_bndy_type,             &
            siy_flux_monthly%filename,           siy_flux_monthly%input%file_fmt)
      endif
      call interpolate_forcing(INTERP_WORK,     &
         siy_flux_monthly%DATA(:,:,:,:,1:12), &
         siy_flux_monthly%data_time,         siy_flux_monthly%interp_type, &
         siy_flux_monthly%data_time_min_loc, siy_flux_monthly%interp_freq, &
         siy_flux_monthly%interp_inc,        siy_flux_monthly%interp_next, &
         siy_flux_monthly%interp_last,       0)
      ...
      if (first_call) then
         ...
         do n = 1, ndep_shr_stream_var_cnt
            ...
            if (n == ndep_shr_stream_nh_ind) &
               ndep_shr_stream_fldList = trim(ndep_shr_stream_fldList) /&
                  &/ 'NHx_deposition'
            if (n == ndep_shr_stream_po_ind) &
               ndep_shr_stream_fldList = trim(ndep_shr_stream_fldList) /&
                  &/ 'POx_deposition'
            if (n == ndep_shr_stream_si_ind) &
               ndep_shr_stream_fldList = trim(ndep_shr_stream_fldList) /&
                  &/ 'SIy_deposition'
...
!-----------------------------------------------------------------------
   ...
   do iblock = 1,nblocks_clinic
      ...
      if (tavg_requested(tavg_POx_FLUX)) then
         call accumulate_tavg_field(STF_MODULE(:,:,po4_ind,iblock),  &
                                    tavg_POx_FLUX,iblock,1)
      endif
      if (tavg_requested(tavg_SIy_FLUX)) then
         call accumulate_tavg_field(STF_MODULE(:,:,sio3_ind,iblock),  &
                                    tavg_SIy_FLUX,iblock,1)
      endif
