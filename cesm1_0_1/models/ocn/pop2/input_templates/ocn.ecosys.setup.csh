...
else if ($OCN_GRID == bs2v1) then

   set IC_file  = $DIN_LOC_ROOT/$relpath/ic/IC_ecosystem_bs2v1_with_atm.nc
   set IC_fmt   = nc
   set ALK_scale_factor = 1.0
   set DIC_scale_factor = 1.0
   set O2_scale_factor  = 1.0
   set DST_file   = $DIN_LOC_ROOT/$relpath/forcing/dst79_ecosystem_bs2v1.nc
   set fesed_file = $DIN_LOC_ROOT/$relpath/forcing/fesed_ecosystem_bs2v1.nc
   if ($OCN_TRANSIENT == unset) then
     set ndep_file  = $DIN_LOC_ROOT/$relpath/forcing/ndep_ecosystem_bs2v1_with_atm.nc
   endif
   if ($OCN_TRANSIENT == 1850-2000) then
     set ndep_file = $DIN_LOC_ROOT/$relpath/forcing/ndep_ecosystem_bs2v1_with_atm.nc
   endif
...
if ($OCN_TRANSIENT == unset) then
   ...
   pox_flux_monthly_input%filename        = '$ndep_file_nml'
   pox_flux_monthly_input%file_fmt        = 'nc'
   pox_flux_monthly_input%file_varname    = 'POx_deposition'
   pox_flux_monthly_input%scale_factor    = 3.2289e+06  ! kgN/m^2/sec -> nmolN/cm^2/sec

   siy_flux_monthly_input%filename        = '$ndep_file_nml'
   siy_flux_monthly_input%file_fmt        = 'nc'
   siy_flux_monthly_input%file_varname    = 'SIy_deposition'
   siy_flux_monthly_input%scale_factor    = 3.56e+06  ! kgN/m^2/sec -> nmolN/cm^2/sec
EOF
endif
...
EOF
  ...
  if ($n_tavg_streams_tracer > 0) then
    cat >> $POP2_DOCDIR/$module.tavg << EOF
tavg_freq_opt             = 'never'          'never'
tavg_freq                 =  1              1
tavg_stream_filestrings   = 'ecosystem.stream1'  'ecosystem.stream2'
tavg_file_freq_opt        = 'never'        'never'
tavg_file_freq            =  1              1
