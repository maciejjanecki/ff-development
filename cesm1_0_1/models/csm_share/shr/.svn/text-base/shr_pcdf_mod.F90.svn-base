...
contains
!===============================================================================
subroutine shr_pcdf_readwrite(type,filename,mpicom,gsmap,dof,clobber,cdf64, &
  ...
  integer(IN)   :: lpio_root, lpio_stride, lpio_ntasks, lpio_type
...
!  lpio_type = iotype_pnetcdf
  lpio_type = iotype_netcdf
  lpio_root = 0
  lpio_stride = 4
  lpio_ntasks = max(1,ntasks/lpio_stride)
  ...
  if (iam == 0) then
     ...
     write(shr_log_unit,*) subname,' pio_type   = ',lpio_type
     write(shr_log_unit,*) subname,' pio_root   = ',lpio_root
     write(shr_log_unit,*) subname,' pio_ntasks = ',lpio_ntasks
     write(shr_log_unit,*) subname,' pio_stride = ',lpio_stride
     call shr_sys_flush(shr_log_unit)
  endif

  call pio_init(iam,mpicom,lpio_ntasks,0,lpio_stride, PIO_REARR_BOX,iosystem,base=lpio_root)
  ...
  if (readtype) then
     ...
     nmode = pio_nowrite
     rcode = pio_openfile(iosystem, fid, lpio_type, trim(filename), nmode)
  else
     ...
     if (lclobber .or. .not.exists) then
        ...
        rcode = pio_createfile(iosystem, fid, lpio_type, trim(filename), nmode)
     else
        nmode = pio_write
        if (lcdf64) nmode = ior(nmode,PIO_64BIT_OFFSET)
        rcode = pio_openfile(iosystem, fid, lpio_type, trim(filename), nmode)
  ...
  call pio_finalize(iosystem,rcode)
